...
Config: loaded via ROS 2 parameter system (networking.launch.py).
  ...
  - skyhunter_comm.yaml (skyhunter_comm):  RF model, delay, packet loss, rates
"""
...
class WiFi6MeshSim(Node):
    ...
    def __init__(self):
        ...
        # Discovery — from skyhunter_comm.yaml
        ...
        # ── Subscriber ────────────────────────────────────────────────────────
        self.create_subscription(JammedLinks, '/jammed_links', self.jammed_links_callback, 10)
    ...
    def get_robot_distance(self, robot_a: int, robot_b: int) -> float:
        try:
            ...
            # If ID is 1, use global 'base_link'. Else use 'SH_XX/base_link'
            frame_a = "base_link" if robot_a == 1 else f"SH_{robot_a:02d}/base_link"
            frame_b = "base_link" if robot_b == 1 else f"SH_{robot_b:02d}/base_link"

            t1 = self.tf_buffer.lookup_transform('map', frame_a, rclpy.time.Time())
            t2 = self.tf_buffer.lookup_transform('map', frame_b, rclpy.time.Time())
    ...
    def jammed_links_callback(self, msg: JammedLinks):
        # Build new dict first, then replace atomically — avoids brief window
        # where jams is empty between clear() and repopulate
        new_jams = {}
        for link in msg.jammed_links:
            a   = int(link.robot_a)
            b   = int(link.robot_b)
            key = (min(a, b), max(a, b))
            new_jams[key] = link.attenuation_db
        self.jams = new_jams
