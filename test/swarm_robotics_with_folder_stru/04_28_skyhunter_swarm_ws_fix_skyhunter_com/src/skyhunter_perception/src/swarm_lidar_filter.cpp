...
// // =========================================================================
// // --- TACTICAL TERRAIN CONSTANTS (SOLVES BOTH HILLS AND BRICKS) ---
// // =========================================================================
// const double GRID_RES = 0.20;          // 20cm grid cells (smooths out hill noise)
// const double GRID_SIZE_M = 40.0;       
// const int GRID_CELLS = int(GRID_SIZE_M / GRID_RES); 

// // A 20-degree slope rises 7.3cm across 20cm. A brick rises 10cm. 
// // 8.5cm is the mathematical sweet spot.
// const float MAX_VARIANCE = 0.085f; 
// // =========================================================================
...
//     float max_z = -999.0f;
//     int count = 0;
//     bool is_obstacle_face = false; // Hits the vertical wall of the brick
//     bool is_obstacle_top = false;  // Hits the flat top of the brick
// };

// SwarmLidarFilter::SwarmLidarFilter() : Node("swarm_lidar_filter") {
//     this->declare_parameter<double>("stealth_radius", 1.2);
//     filter_radius_sq_ = std::pow(this->get_parameter("stealth_radius").as_double(), 2);
...
//     RCLCPP_INFO(this->get_logger(), "Leader Terrain Engine: 20-Deg Hills + 10cm Brick Tops Active.");
// }
...
//     // PASS 2: Identify Flat Brick Tops vs Slopes
//     for (int y = 0; y < GRID_CELLS; ++y) {
//         for (int x = 0; x < GRID_CELLS; ++x) {
//             int idx = y * GRID_CELLS + x;
//             if (grid[idx].count == 0) continue;
            
//             // Check 1: Does the cell contain a vertical face?
//             if ((grid[idx].max_z - grid[idx].min_z) > MAX_VARIANCE) {
//                 grid[idx].is_obstacle_face = true;
//             }

//             // Check 2: Is this flat cell significantly higher than its neighbor? (The brick top)
//             int dx[] = {0, 0, -1, 1};
//             int dy[] = {-1, 1, 0, 0};
//             for (int i = 0; i < 4; ++i) {
//                 int nx = x + dx[i];
...
//                     if (grid[n_idx].count > 0) {
//                         // If my floor is 8.5cm higher than my neighbor's floor, I am standing on a brick.
//                         if (grid[idx].min_z - grid[n_idx].min_z > MAX_VARIANCE) {
//                             grid[idx].is_obstacle_top = true;
//                         }
//                     }
//                 }
//             }
...
//             total_cells++;
//             if (grid[y * GRID_CELLS + x].count < 2) empty_cells++;
//         }
//     }
//     if ((float)empty_cells / total_cells > 0.70) cliff_detected = true;

//     // PASS 3: Route points to Nav2
//     pcl::PointCloud<pcl::PointXYZ>::Ptr obstacle_cloud(new pcl::PointCloud<pcl::PointXYZ>);
//     pcl::PointCloud<pcl::PointXYZ>::Ptr ground_cloud(new pcl::PointCloud<pcl::PointXYZ>);
...
//         if (gx >= 0 && gx < GRID_CELLS && gy >= 0 && gy < GRID_CELLS) {
//             int idx = gy * GRID_CELLS + gx;
//             bool point_is_obstacle = false;

//             if (grid[idx].is_obstacle_top) {
//                 // The whole cell is the flat top of a brick. All points are obstacles.
//                 point_is_obstacle = true;
//             } else if (grid[idx].is_obstacle_face) {
//                 // It's the edge of a brick. Only the points 4cm above the ground are obstacles.
//                 if (p.z > grid[idx].min_z + 0.04) {
//                     point_is_obstacle = true;
//                 }
//             }

//             if (point_is_obstacle) {
//                 #pragma omp critical
//                 obstacle_cloud->push_back(p);
//             } else {
//                 #pragma omp critical
