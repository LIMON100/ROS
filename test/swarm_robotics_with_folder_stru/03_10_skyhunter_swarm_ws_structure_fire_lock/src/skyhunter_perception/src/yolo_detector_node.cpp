...
void YoloDetectorNode::image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        // --- 1. TACTICAL SLEEP MODE (Fix for rqt_image_view Jamming) ---
        // if (!is_leader_ && last_swarm_msg_.swarm_state < 3) {
        //     // Draw a standby message so the user knows the node is alive
        //     cv::putText(cv_ptr->image, "MODE: PATROL (STANDBY - ZERO GPU USAGE)", 
        //                 cv::Point(20, 30), 0, 0.7, cv::Scalar(0, 255, 0), 2);
        //     pub_overlay_->publish(*(cv_ptr->toImageMsg()));
        //     return; // Exit BEFORE running the heavy YOLO engine
        // }
        ...
        if (ai_sees_target) {
            ...
            for (size_t i = 0; i < tracked_targets.size(); ++i) {
                ...
                // if ((int)i == best_idx) {
                //     std::vector<float> vel = tracked_targets[i]->getVelocity();
                //     float pred_cx = (r.x() + r.width()/2.0f) + (vel[0] * 0.5f);
                //     float pred_cy = (r.y() + r.height()/2.0f) + (vel[1] * 0.5f);
                ...
                if ((int)i == best_idx) {
                    ...
                    double i_gain_pan = -0.15; // NEW: Pushes pan to exact zero

                    double p_gain_tilt = 0.8; 
                    double i_gain_tilt = 0.1;  // NEW: Pushes tilt to exact zero
            ...
            // --- FIX FOR ISSUE 2: FOLLOWER POINTING WRONG WAY ---
            current_state = COMBAT;
            double tx = last_swarm_msg_.target_pos.x;
            double ty = last_swarm_msg_.target_pos.y;
            double my_x = latest_odom_.pose.pose.position.x;
...
//         auto now = this->get_clock()->now();
//         for (const auto& [f_ns, time] : follower_confirmations_) {
//             // If a follower voted in the last 1.5 seconds, it counts!
//             if ((now - time).seconds() < 1.5) active_votes++;
//         }
//     }

//     if (ai_sees_target) {
//         // --- FOLLOWER VOTING LOGIC ---
//         // If I am a follower, I see a target, and the swarm is in combat mode: VOTE YES!
//         if (!is_leader_ && (last_swarm_msg_.swarm_state == 3 || last_swarm_msg_.swarm_state == 4)) {
//             std_msgs::msg::String vote; vote.data = my_ns_;
//             pub_confirm_->publish(vote);
...
//             // Kept original class label logic
//             // det.label = (tracked_targets[i].class_id == 74) ? "frisbee" : "person"; 
//             det.label = "person";
...
//             //     // --- LEADER STATE BROADCAST (WITH DOUBLE LOCK) ---
//             //     if (is_leader_) {
//             //         skyhunter_msgs::msg::LeaderState state_msg;
//             //         state_msg.header.stamp = this->get_clock()->now();
//             //         double r_yaw = tf2::getYaw(latest_odom_.pose.pose.orientation);
...
//             //                 current_state = COMBAT;
//             //                 cv::putText(cv_ptr->image, "WAITING FOR SWARM CONFIRMATION...", cv::Point(20, 60), 0, 0.7, cv::Scalar(0, 165, 255), 2);
//             //             }
//             //         } else {
//             //             state_msg.swarm_state = 0;
//             //         }
...
//                 if (is_leader_) {
//                     if (active_votes >= 1) {
//                         // 2 ROBOTS SEE IT (Leader + 1 Follower) -> RED BOX
//                         target_color = cv::Scalar(0, 0, 255); // Red
//                         target_text = "TARGET CONFIRMED " + label;
//                         current_state = ENGAGED;
//                     } else {
//                         // ONLY LEADER SEES IT -> YELLOW BOX (Waiting)
//                         target_color = cv::Scalar(0, 255, 255); // Yellow
//                         target_text = "WAITING FOR VOTES " + label;
//                         current_state = COMBAT;
//                     }
//                 } else {
//                     // FOLLOWER VIEW -> ORANGE BOX
//                     target_color = cv::Scalar(0, 165, 255); // Orange
