...
void RobustFollower::control_loop()
{
  ...
  // =========================================================================
  //  3D-SAFE YAW EXTRACTION (fixes pitch/roll on hills)
  // =========================================================================
  tf2::Quaternion q_my, q_lead;
  tf2::fromMsg(tf_now.transform.rotation, q_my);
  tf2::fromMsg(last_leader_msg_.pose.orientation, q_lead);
  ...
  if (is_combat) {
    ...
    target_y = ty + 5.0 * std::sin(final_angle);
      ...
      // =====================================================================
      // CONVOY PROTOCOL: STRICT BREADCRUMB FOLLOWING (Fixes falling off cliffs)
      // =====================================================================
      double target_distance_behind = std::abs(current_off_back);
      geometry_msgs::msg::Point target_crumb = leader_breadcrumbs_.front(); // Default to oldest
      double accumulated_dist = 0.0;
      
      // Traverse the history backwards to find the exact point N meters behind the leader
      ...
      // Override Boids Repulsion during dangerous narrow cliff driving
