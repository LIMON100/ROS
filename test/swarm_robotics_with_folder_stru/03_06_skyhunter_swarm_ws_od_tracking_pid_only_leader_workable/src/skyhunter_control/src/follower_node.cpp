...
// 03_06 - with tracking and follower getting detection command
#include <chrono>
#include <cmath>
#include <memory>
#include <string>
...
private:
  ...
  void control_loop() {
    ...
    if (last_leader_msg_.swarm_state == 3 && last_leader_msg_.target_locked) {
        ...
        // 3. Calculate "Cue" Angle to Target
        double dx = last_leader_msg_.target_pos.x - my_x;
        double dy = last_leader_msg_.target_pos.y - my_y;
        double global_angle = std::atan2(dy, dx);
        
        // 4. Convert to Gimbal Angle (Relative)
        double pan_angle = global_angle - my_yaw;
        // Normalize to -PI to PI
        while (pan_angle > M_PI) pan_angle -= 2 * M_PI;
        while (pan_angle < -M_PI) pan_angle += 2 * M_PI;

        // 5. Send Command ("The Cue")
        std_msgs::msg::Float64 pan_msg;
        pan_msg.data = pan_angle;
        pub_pan_->publish(pan_msg);

        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "COMBAT: Cueing Gimbal to %.2f deg", pan_angle * 57.3);
        return; // Skip the rest of the navigation loop
    }
