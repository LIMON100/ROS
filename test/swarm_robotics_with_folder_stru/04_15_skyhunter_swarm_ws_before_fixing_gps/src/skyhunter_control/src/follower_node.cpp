...
void RobustFollower::control_loop()
{
  ...
  double dx = last_leader_msg_.pose.position.x - my_x;
  double dy = last_leader_msg_.pose.position.y - my_y;
  double dz = last_leader_msg_.pose.position.z - tf_now.transform.translation.z; // Use Z!
  double dist_to_leader = std::sqrt(dx*dx + dy*dy + dz*dz);
