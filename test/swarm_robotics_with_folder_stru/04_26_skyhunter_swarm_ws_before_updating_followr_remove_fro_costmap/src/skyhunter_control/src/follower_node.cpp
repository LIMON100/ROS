...
void RobustFollower::scan_cb(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  for (const auto& p : aligned_cloud->points) {
      ...
      if (gx >= 0 && gx < DIM && gy >= 0 && gy < DIM) {
          ...
          if (height_diff > 0.10) { 
              if (p.z > grid[idx].min_z + 0.05) { // Ensure we aren't just picking up the floor point
...
void RobustFollower::control_loop()
{
  ...
  // =========================================================================
  // STEP 3: THE "ANTI-STUCK" SLIP DETECTOR
  // =========================================================================
  ...
  if (has_gps_) {
      total_ticks++;
      // Only count ticks where the brain is ACTIVELY trying to drive the motors
      if (std::abs(last_cmd_vel_x_) > 0.2) high_cmd_ticks++;
      ...
      if (dt_stuck >= 3.0) { 
          if (last_lat != 0.0 && last_lon != 0.0) {
              ...
              // --- DIAGNOSTIC LOG: THIS WILL SHOW YOU THE GPS TRUTH ---
              RCLCPP_INFO(this->get_logger(), "GPS CHECK: Physical Move: %.3fm | Cmd Active: %d/%d", 
                          physical_dist, high_cmd_ticks, total_ticks);

              //  Lowered threshold from 0.3 to 0.4 to account for jitter, 
              // but increased the command requirement.
              if ((float)high_cmd_ticks / total_ticks > 0.7 && physical_dist < 0.4 && !is_reversing) {
                  RCLCPP_ERROR(this->get_logger(), "!!! GPS PHYSICAL STUCK !!! Moved %.2fm. Reversing...", physical_dist);
                  is_reversing = true;
  ...
  if (is_reversing) {
      if ((this->get_clock()->now() - reverse_start_time).seconds() < 2.0) {
          ...
          rev_cmd.linear.x = -0.8; // Reverse aggressively
          rev_cmd.angular.z = 0.8; // Twist hard to break out of rocks/ruts
          pub_cmd_->publish(rev_cmd);
          return; // SKIP the rest of the control loop!
      } else {
          is_reversing = false; // Done reversing, resume normal driving
      }
  }
  ...
  if (has_swarm_) {
    for (const auto& other_pose : swarm_poses_.poses) {
      ...
      // --- INCREASE TO 0.6m TO IGNORE GPS GHOSTS ---
      if (d < 0.6) continue; 
  ...
  // FRONT CLEARANCE + TTC + LOCAL SAMPLING 
  double min_front_dist = 10.0;
  for (const auto& p : obstacle_points_) {
    float angle = std::atan2(p.y, p.x);
    if (std::abs(angle) < 0.7) {
