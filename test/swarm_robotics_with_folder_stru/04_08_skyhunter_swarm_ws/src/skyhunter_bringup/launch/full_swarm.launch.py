...
def launch_setup(context, *args, **kwargs):
    ...
    nodes_to_start.append(TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(pkg_tin3_navigation, 'launch', 'ekf.launch.py')),
    ...
    # 4. Logic & Perception (Delay 15 seconds)
    nodes_to_start.append(TimerAction(
        period=15.0,
        actions=[
            Node(package='skyhunter_control', executable='leader_node', output='screen', parameters=[{'use_sim_time': True, 'initial_formation': int(formation_type)}], remappings=[('odom', '/odom_filtered'), ('leader_state', '/leader_state')]),
            Node(package='skyhunter_perception', executable='yolo_detector_node', name='leader_yolo_node', output='screen', parameters=[{'robot_ns': ''}]),
            Node(package='skyhunter_perception', executable='swarm_lidar_filter', name='leader_lidar_filter', output='screen', parameters=[{'use_sim_time': True, 'stealth_radius': 1.0}], remappings=[('scan/points', '/scan/points'), ('scan/points_filtered', '/scan/points_filtered')]),
            Node(package='skyhunter_control', executable='leadership_manager', name='leadership_manager_R1', parameters=[{'robot_int_id': 1}], remappings=[('leader_state', '/leader_state'), ('/swarm/virtual_leader/state', '/swarm/virtual_leader/state')])
        ]
    ))
