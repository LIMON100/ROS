...
void RobustFollower::scan_cb(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  geometry_msgs::msg::TransformStamped tf_map_to_local;
  try {
    tf_map_to_local = tf_buffer_->lookupTransform(my_frame_, "map", tf2::TimePointZero);
    tf2::doTransform(leader_map, leader_local, tf_map_to_local);
  } catch (...) {
    return; // Wait for TF to be ready
  }

  double lx_l = leader_local.point.x;
  double ly_l = leader_local.point.y;

  // --- NEW: Transform all teammates to local frame to ignore them ---
  std::vector<std::pair<double, double>> local_teammates;
  if (has_swarm_) {
    for (const auto& pose : swarm_poses_.poses) {
      geometry_msgs::msg::PointStamped p_map, p_local;
      p_map.header.frame_id = "map";
      p_map.point = pose.position;
      tf2::doTransform(p_map, p_local, tf_map_to_local);
      
      // Ignore self (dist to local origin > 0.1)
      if (p_local.point.x * p_local.point.x + p_local.point.y * p_local.point.y > 0.1) {
        local_teammates.push_back({p_local.point.x, p_local.point.y});
      }
    }
  }
...
#pragma omp for nowait
    for (size_t i = 0; i < raw_cloud->size(); i += 10) {
      const auto& p = raw_cloud->points[i];
      // --- FIX 1: Ignore ground slopes / hills ---
      if (p.z < 0.4 || p.z > 1.5) continue; 
      ...
      // --- FIX 2: Filter out other teammates so followers don't brake! ---
      bool hit_teammate = false;
      for (const auto& tm : local_teammates) {
          if (std::hypot(p.x - tm.first, p.y - tm.second) < 1.2) {
              hit_teammate = true;
              break;
          }
      }
      if (hit_teammate) continue;
...
void RobustFollower::control_loop()
{
  ...
  if (is_combat) {
      // === SMART FIRE NET / RELATIVE ENCIRCLE MODE ===
      int my_id = 2;
      size_t underscore_pos = my_ns_.find("_");
      if (underscore_pos != std::string::npos && underscore_pos + 1 < my_ns_.length()) {
          try { my_id = std::stoi(my_ns_.substr(underscore_pos + 1)); } catch (...) {}
      ...
      // Base Angle: The angle FROM the Target TO the Leader
      double base_angle = std::atan2(ly - ty, lx - tx);

      // NON-CROSSING SLOTS: Left robots go left (-), Right robots go right (+)
      double angle_offset = 0.0;
      if (my_id == 2) angle_offset = -M_PI / 3.0;             // SH_02: Left Flank (-60 deg)
      else if (my_id == 3) angle_offset = M_PI / 3.0;         // SH_03: Right Flank (+60 deg)
      else if (my_id == 4) angle_offset = -2.0 * M_PI / 3.0;  // SH_04: Far Left (-120 deg)
      else if (my_id == 5) angle_offset = 2.0 * M_PI / 3.0;   // SH_05: Far Right (+120 deg)
      else if (my_id == 6) angle_offset = -M_PI;              // SH_06: Rear Target (-180 deg)
      else if (my_id == 7) angle_offset = M_PI;               // SH_07: Rear Target (+180 deg)

      // Calculate final target slot on the 5m circle
      ...
      RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
          "FIRE NET: SH_%02d moving to slot (Offset: %.0f°) @ (%.1f, %.1f)",
          my_id, angle_offset * 180.0 / M_PI, target_x, target_y);
  } 
  // else {
  //     // === NORMAL FORMATION (original code) ===
  //     if (last_leader_msg_.next_waypoints.size() >= 2) {
  ...
  //     }
      ...
      if (last_leader_msg_.next_waypoints.size() >= 2) {
          ...
          // --- FIX 3: Prevent Atan2(0,0) snap when Leader reaches the final goal ---
          if (std::abs(path_dx) < 0.01 && std::abs(path_dy) < 0.01) {
              static double smoothed_yaw = l_yaw;
              double angle_diff = normalize_angle(l_yaw - smoothed_yaw);
              smoothed_yaw = normalize_angle(smoothed_yaw + (angle_diff * 0.1));
