...
class RobotDiscovery:
    ...
    def discover_from_tf(self, tf_buffer) -> List[int]:
        ...
        try:
            ...
            # 1. Find all followers (SH_02, SH_03, etc)
            for match in self._pattern.finditer(frames_yaml):
                robot_id = int(match.group(1))
                robots.add(robot_id)

            # 2. CRITICAL FIX: Explicitly check for the Global Leader (Robot 1)
            # If the raw "base_footprint" (or whatever base_link is) exists, add ID 1.
            if f"{self.config.base_link}:" in frames_yaml:
                robots.add(1)
    ...
    def get_robot_frame(self, robot_id: int) -> str:
        """Generate full frame name for robot."""
        # IF LEADER (1): Frame is just 'base_footprint' (no prefix)
        if int(robot_id) == 1:
            return self.config.base_link
            
        # IF FOLLOWER (2+): Frame is 'SH_02/base_footprint'
        return f"{self.config.prefix}{int(robot_id):02d}/{self.config.base_link}"
