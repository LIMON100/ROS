...
class MeshVisualizer(Node):
    ...
    def metrics_cb(self, msg):
        ...
        # --- NEW: Get params from the node (matching your YAML) ---
        common_frame = "map"  # Match your YAML
        prefix = "SH_"
        base_link = "base_footprint"

        for i, link in enumerate(msg.links):
            # We only draw if the link is physically connected
            ...
            # --- TACTICAL FRAME LOGIC ---
            # Robot A
            id_a = int(link.robot_a)
            frame_a = base_link if id_a == 1 else f"{prefix}{id_a:02d}/{base_link}"
            
            # Robot B
            id_b = int(link.robot_b)
            frame_b = base_link if id_b == 1 else f"{prefix}{id_b:02d}/{base_link}"

            try:
                tf_a = self.tf_buffer.lookup_transform(common_frame, frame_a, rclpy.time.Time())
                tf_b = self.tf_buffer.lookup_transform(common_frame, frame_b, rclpy.time.Time())
                
                m.points = [
                    Point(x=tf_a.transform.translation.x, y=tf_a.transform.translation.y, z=0.5),
                    Point(x=tf_b.transform.translation.x, y=tf_b.transform.translation.y, z=0.5)
                ]
            except Exception:
                continue
            
            # Style the line based on RSSI
            m.scale.x = 0.05
