...
Config: skyhunter_comm/config/wifi6_mesh_sim.yaml (jammer_service section)
"""
...
class JammerService(Node):
    ...
    def __init__(self):
        ...
        # Load config from YAML directly
        params = self._load_yaml_config()
        publish_rate = params['publish_rate_hz']
        self.default_attenuation = params['default_attenuation_db']
    ...
    def _load_yaml_config(self) -> dict:
        """Load config from YAML file directly."""
        pkg_networking = get_package_share_directory('skyhunter_comm')
        yaml_path = os.path.join(pkg_networking, 'config', 'wifi6_mesh_sim.yaml')
        
        with open(yaml_path) as f:
            params = yaml.safe_load(f)['jammer_service']['ros__parameters']
        
        self.get_logger().info(f'Loaded config from: {yaml_path}')
        return params
