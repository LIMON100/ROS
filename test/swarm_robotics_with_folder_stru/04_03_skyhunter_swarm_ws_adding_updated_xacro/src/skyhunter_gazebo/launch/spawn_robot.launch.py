...
#     # If altitude is more than 1 meter away from 0, assume a terrain world
#     if abs(z_float) > 1.0:
#         map_z_offset = str(z_float)
#     else:
#         map_z_offset = '0' 
...
#         arguments=[
#             str(x_value), str(y_value), map_z_offset, 
#             '0', '0', str(yaw_value),
#             'map', odom_frame
#         ],
...
#         DeclareLaunchArgument("lidar_mode", default_value="low"),
...
def spawn_robot(context: LaunchContext, namespace: LaunchConfiguration, pose: LaunchConfiguration, lidar_mode: LaunchConfiguration, use_sim_time: LaunchConfiguration):
    pkg_tin3_description = get_package_share_directory("skyhunter_description")
    pkg_tin3_gz_simulation = get_package_share_directory("skyhunter_gazebo")
    ...
    with open(os.path.join(pkg_tin3_gz_simulation, "config", "robot_params.yaml")) as f:
        ROBOT_PREFIX = yaml.safe_load(f)["/**"]["ros__parameters"]["robot"]["prefix"]

    pose_parts = pose_str.split()
    x_value, y_value, z_value = pose_parts[0], pose_parts[1], pose_parts[2]
    roll_value, pitch_value, yaw_value = pose_parts[3], pose_parts[4], pose_parts[5]

    robot_desc = xacro.process(os.path.join(pkg_tin3_description, "urdf", "robot.urdf.xacro"), mappings={"robot_ns": robot_ns, "lidar_mode": context.perform_substitution(lidar_mode)})

    robot_gazebo_name = robot_ns if robot_ns else ROBOT_PREFIX.rstrip('_')
    node_name_prefix = robot_ns + "_" if robot_ns else ""

    robot_state_publisher = Node(
        namespace=robot_ns, package="robot_state_publisher", executable="robot_state_publisher",
        name="robot_state_publisher", parameters=[{"use_sim_time": use_sim_time_value, "robot_description": robot_desc, "frame_prefix": robot_ns + "/" if robot_ns else ""}],
        remappings=[("/tf", "/tf"), ("/tf_static", "/tf_static")]
    )

    spawn_entity = Node(
        namespace=robot_ns, package="ros_gz_sim", executable="create", name="ros_gz_sim_create",
        arguments=["-topic", "robot_description", "-name", robot_gazebo_name, "-x", x_value, "-y", y_value, "-z", z_value, "-R", roll_value, "-P", pitch_value, "-Y", yaw_value]
    )

    bridge_prefix = "/" + robot_ns if robot_ns else ""
    topic_bridge = Node(
        package="ros_gz_bridge", executable="parameter_bridge", name=node_name_prefix + "parameter_bridge",
        arguments=[
            bridge_prefix + "/cmd_vel@geometry_msgs/msg/Twist]gz.msgs.Twist",
            bridge_prefix + "/gimbal/pan_cmd@std_msgs/msg/Float64]gz.msgs.Double",
            bridge_prefix + "/gimbal/tilt_cmd@std_msgs/msg/Float64]gz.msgs.Double",
        ...
        remappings=[(bridge_prefix + "/tf", "/tf")],
        parameters=[{"use_sim_time": use_sim_time_value}, {"qos_overrides./tf.publisher.reliability": "reliable"}, {"qos_overrides./tf_static.publisher.durability": "transient_local"}]
    )

    # --- RESTORED: Map to Odom Static Transform (Guarantees Nav2 won't crash) ---
    map_z_offset = str(float(z_value)) if abs(float(z_value)) > 1.0 else '0'
    odom_to_world = Node(
        package='tf2_ros', executable='static_transform_publisher', name=node_name_prefix + 'odom_to_world',
        arguments=[x_value, y_value, map_z_offset, '0', '0', yaw_value, 'map', robot_ns + '/odom' if robot_ns else 'odom'],
        parameters=[{"use_sim_time": use_sim_time_value}]
    )
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument("robot_ns", default_value=""), DeclareLaunchArgument("pose", default_value="0 0 0.5"),
        DeclareLaunchArgument("lidar_mode", default_value="low"), DeclareLaunchArgument("use_sim_time", default_value="true"),
        OpaqueFunction(function=spawn_robot, args=[LaunchConfiguration("robot_ns"), LaunchConfiguration("pose"), LaunchConfiguration("lidar_mode"), LaunchConfiguration("use_sim_time")])
    ])