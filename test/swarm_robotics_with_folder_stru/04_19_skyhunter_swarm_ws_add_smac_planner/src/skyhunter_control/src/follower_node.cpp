...
void RobustFollower::scan_cb(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  // 1. Transform to base_footprint (Align with gravity)
  geometry_msgs::msg::TransformStamped tf_lidar_to_base;
  try { tf_lidar_to_base = tf_buffer_->lookupTransform(my_frame_, msg->header.frame_id, tf2::TimePointZero); } 
  catch (...) { return; }
  Eigen::Affine3d transform = tf2::transformToEigen(tf_lidar_to_base.transform);
  ...
  // --- TACTICAL GRID PARAMETERS ---
  const double RES = 0.2; // 20cm cells
  const int DIM = 60;     // 8mx8m area
  struct Cell { float min_z = 99.0; float max_z = -99.0; int count = 0; };
  std::vector<Cell> grid(DIM * DIM);

  // First pass: Populate the elevation grid
  for (const auto& p : aligned_cloud->points) {
      int gx = (p.x / RES) + (DIM/2);
      int gy = (p.y / RES) + (DIM/2);
  ...
  // Second pass: Categorize points
  for (const auto& p : aligned_cloud->points) {
      int gx = (p.x / RES) + (DIM/2);
      int gy = (p.y / RES) + (DIM/2);
      int idx = gy * DIM + gx;

      // --- A. CLIFF DETECTION (Check if ground exists) ---
      if (p.x > 1.0 && p.x < 3.5 && std::abs(p.y) < 0.8) {
          if (p.z > -1.5 && p.z < 0.8) ground_front++;
      }

      // --- B. SMART OBSTACLE FILTER ---
      if (gx >= 0 && gx < DIM && gy >= 0 && gy < DIM) {
          // Calculate height difference in this tiny 20cm square
          float height_diff = grid[idx].max_z - grid[idx].min_z;

          // If the point is > 0.25m higher than the lowest point in its cell, it's an OBSTACLE
          // This works on hills because in a 20cm cell, the floor is flat.
          if (height_diff > 0.25) { 
              if (p.z > grid[idx].min_z + 0.15) { // Ensure we aren't just picking up the floor point
                  
                  // Standard Ignore Self/Leader filters
                  if ((p.x * p.x + p.y * p.y) < 0.35) continue;
                  if (std::hypot(p.x - leader_local.point.x, p.y - leader_local.point.y) < 1.2) continue;
  ...
  // --- C. EMERGENCY BRAKE (The Void) ---
  if (ground_front < 10 && last_leader_msg_.formation_type != 1) {
      for (double vy = -1.0; vy <= 1.0; vy += 0.2) obs.push_back(pcl::PointXYZ(1.2, vy, 0.5));
      RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 500, "TERRAIN VOID! Braking.");
  }
...
void RobustFollower::control_loop()
{
  ...
  // =========================================================================
  // STEP 3: THE "STUCK & SLIP" DETECTOR (Auto-Reverse)
  // =========================================================================
  static rclcpp::Time last_stuck_check_time = this->get_clock()->now();
  static double last_x = my_x;
  static double last_y = my_y;
  static double stuck_timer = 0.0;
  static bool is_reversing = false;
  static rclcpp::Time reverse_start_time;
  static double last_cmd_speed = 0.0; // Remembers what we told the motors to do last tick

  double dt_stuck = (this->get_clock()->now() - last_stuck_check_time).seconds();
  if (dt_stuck >= 0.5) { // Evaluate actual physical movement every 0.5 seconds
      double dist_moved = std::hypot(my_x - last_x, my_y - last_y);
      double actual_speed = dist_moved / dt_stuck;

      // If we COMMANDED speed > 0.2 m/s, but ACTUALLY moved < 0.05 m/s -> Wheels are slipping!
      if (!is_reversing && last_cmd_speed > 0.2 && actual_speed < 0.05) {
          stuck_timer += dt_stuck;
      } else {
          stuck_timer = 0.0; // We are moving fine, reset timer
      }
      last_x = my_x;
      last_y = my_y;
  ...
  if (stuck_timer > 3.0 && !is_reversing) {
      RCLCPP_ERROR(this->get_logger(), "!!! STUCK DETECTED !!! Wheels spinning but no movement. Executing Auto-Reverse!");
      is_reversing = true;
      reverse_start_time = this->get_clock()->now();
      stuck_timer = 0.0;
  }

  // EXECUTE THE REVERSE MANEUVER
  if (is_reversing) {
      if ((this->get_clock()->now() - reverse_start_time).seconds() < 2.0) {
          geometry_msgs::msg::Twist reverse_cmd;
          reverse_cmd.linear.x = -0.8; // Drive backwards fast
          reverse_cmd.angular.z = 0.6; // Twist slightly to pop the tracks out of the rut
          pub_cmd_->publish(reverse_cmd);
          
          last_cmd_speed = reverse_cmd.linear.x;
          return; // EXIT EARLY! Do not run the normal formation math while reversing!
      } else {
          is_reversing = false; // Done reversing, resume normal mission
      }
  }
  // =========================================================================
