...
const double GRID_RES = 0.25;          // 25cm grid cells
const double GRID_SIZE_M = 40.0;       // 40m x 40m total grid area
const int GRID_CELLS = int(GRID_SIZE_M / GRID_RES); // 160 x 160 cells
const double SLOPE_THRESHOLD = 0.20;   // Max height difference in a 25cm cell to be "Ground" (Allows ~35 degree slopes)
...
SwarmLidarFilter::SwarmLidarFilter() : Node("swarm_lidar_filter") {
    ...
    RCLCPP_INFO(this->get_logger(), "Tactical Terrain Engine Online. Max Slope Variance: %.2fm", SLOPE_THRESHOLD);
}
