...
public:
    ElevationMapperNode() : Node("elevation_mapper_node")
    {
        ...
        this->declare_parameter<double>("map.length", 12.0); // Slightly larger map
        this->declare_parameter<double>("map.resolution", 0.05);
        this->declare_parameter<double>("max_slope_deg", 35.0); // More tolerant slope
        this->declare_parameter<double>("max_step_height", 0.235);
        this->declare_parameter<double>("max_gap_size", 0.220);
        ...
        grid_map_.add("density_high", 0.0); 
        grid_map_.add("drop_risk", 0.0);
        grid_map_.add("traversability", 0.0); // Initialize with 0 (Safe)
        ...
        RCLCPP_INFO(this->get_logger(), "Elevation Mapper Started. Aggressive Self-Filtering Enabled.");
    }

private:
    void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        // ... (Keep TF prefixing and Transform lookup exactly as they are) ...
        std::string target_frame_id = msg->header.frame_id;
        size_t slash_pos = robot_base_frame_.find('/');
        if (slash_pos != std::string::npos) {
            std::string prefix = robot_base_frame_.substr(0, slash_pos + 1); 
        ...
        for (const auto& point : cloud->points)
        {
            // --- FILTER 1: UNDERGROUND NOISE ---
            // Ignore points significantly below ground (Odom frame Z=0)
            if (point.z < -0.2) continue; 
            if (point.z > 2.0) continue;

            // --- FILTER 2: AGGRESSIVE SELF FILTER ---
            // 1.5m radius to completely ignore robot body/tracks
            double dx = point.x - sensor_x;
            double dy = point.y - sensor_y;
            if ((dx*dx + dy*dy) < (1.5 * 1.5)) continue; 

            // --- FILTER 3: ZERO FILTER ---
            if (std::abs(point.x) < 0.01 && std::abs(point.y) < 0.01) continue;

            grid_map::Index index;
            if (!grid_map_.getIndex(grid_map::Position(point.x, point.y), index)) continue;

            // --- GROUND SNAPPING FIX ---
            float current_elev = grid_map_.at("elevation", index);
            
            // If cell is empty, initialize it.
            if (!std::isfinite(current_elev)) {
                // If point is close to Z=0 (e.g., -0.1 to 0.1), snap to 0.0.
                // This creates a flat baseline for the ground.
                if (std::abs(point.z) < 0.15) {
                    grid_map_.at("elevation", index) = 0.0;
                } else {
                    grid_map_.at("elevation", index) = point.z;
                }
            } 
            // If we found a lower point, update ground
            else if (point.z < current_elev) {
                grid_map_.at("elevation", index) = point.z;
            }

            // --- OBSTACLE DETECTION FIX ---
            // Calculate height above the cell's ground
            float h = point.z - grid_map_.at("elevation", index);

            // UPDATED THRESHOLD:
            // Old: 0.3 -> Caused Ghost Obstacles on bumpy ground
            // New: 0.5 -> Only detects things 50cm tall (Walls, Boxes)
            if (h >= 0.5) {
                grid_map_.at("density_high", index) += 1.0;
            }
            else if (h > 0.1) {
        ...
        // ... (ROI Iterator and Polygon logic stays exactly the same) ...
        grid_map::Polygon roi_polygon;
        roi_polygon.setFrameId(map_frame_);
        double look_ahead = 6.0;
        double half_width = 1.2; 
        double c = cos(yaw);
        double s = sin(yaw);
        auto add_corner = [&](double x, double y) {
            double rx = x * c - y * s + sensor_x;
            double ry = x * s + y * c + sensor_y;
            roi_polygon.addVertex(grid_map::Position(rx, ry));
        };
        add_corner(look_ahead, half_width);   
        add_corner(look_ahead, -half_width);  
        add_corner(-1.0, -half_width); 
        add_corner(-1.0, half_width);  
        ...
        for (grid_map::PolygonIterator iterator(grid_map_, roi_polygon); !iterator.isPastEnd(); ++iterator)
        {
            ...
            // ... (Neighbor analysis for slope/step stays same) ...
            float elevation = grid_map_.at("elevation", *iterator);
            float max_height_diff = 0.0;
            float max_step_down = 0.0; 
            float sum_z = 0.0;
            ...
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    if (dx==0 && dy==0) continue;
                    grid_map::Index neighbor_idx(current_idx(0)+dx, current_idx(1)+dy);
                    if (grid_map_.isValid(neighbor_idx, "elevation")) {
                        float n_elev = grid_map_.at("elevation", neighbor_idx);
                        sum_z += n_elev; sum_sq_z += n_elev*n_elev; count++;
                        float diff = elevation - n_elev;
                        if (std::abs(diff) > max_height_diff) max_height_diff = std::abs(diff);
                        if (diff > max_step_down) max_step_down = diff; 
                    }
            ...
            if (count > 1) {
                float mean = sum_z / count;
                float var = (sum_sq_z / count) - (mean * mean);
                roughness = std::sqrt(std::max(0.0f, var));
            }
            grid_map_.at("roughness", *iterator) = roughness;
            float slope_deg = (atan2(max_height_diff, grid_map_.getResolution()) * 180.0 / M_PI);
            grid_map_.at("slope", *iterator) = slope_deg;
            ...
            // Drop Risk
            if (max_step_down > max_gap) grid_map_.at("drop_risk", *iterator) = 1.0;
            else grid_map_.at("drop_risk", *iterator) = 0.0;
            ...
            if (d_high > 5) cost = 1.0; 
            else if (grid_map_.at("drop_risk", *iterator) > 0.5) cost = 1.0; 
            else if (slope_deg > max_safe_slope) cost = 1.0; 
            else if (max_height_diff > max_step) cost = 1.0; 
            // Relaxed roughness check
            else if (roughness > 0.15) cost = 0.8; 
            else cost = 0.0; 

            grid_map_.at("traversability", *iterator) = cost;
        }
