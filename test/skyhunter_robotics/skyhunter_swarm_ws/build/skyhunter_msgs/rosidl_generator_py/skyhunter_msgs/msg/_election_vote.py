# generated from rosidl_generator_py/resource/_idl.py.em
# with input from skyhunter_msgs:msg/ElectionVote.idl
# generated code does not contain a copyright notice


# Import statements for member types

import builtins  # noqa: E402, I100

import math  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_ElectionVote(type):
    """Metaclass of message 'ElectionVote'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('skyhunter_msgs')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'skyhunter_msgs.msg.ElectionVote')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__election_vote
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__election_vote
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__election_vote
            cls._TYPE_SUPPORT = module.type_support_msg__msg__election_vote
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__election_vote

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class ElectionVote(metaclass=Metaclass_ElectionVote):
    """Message class 'ElectionVote'."""

    __slots__ = [
        '_term',
        '_candidate_id',
        '_voter_id',
        '_fitness_score',
    ]

    _fields_and_field_types = {
        'term': 'uint32',
        'candidate_id': 'string',
        'voter_id': 'string',
        'fitness_score': 'float',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('uint32'),  # noqa: E501
        rosidl_parser.definition.UnboundedString(),  # noqa: E501
        rosidl_parser.definition.UnboundedString(),  # noqa: E501
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.term = kwargs.get('term', int())
        self.candidate_id = kwargs.get('candidate_id', str())
        self.voter_id = kwargs.get('voter_id', str())
        self.fitness_score = kwargs.get('fitness_score', float())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.term != other.term:
            return False
        if self.candidate_id != other.candidate_id:
            return False
        if self.voter_id != other.voter_id:
            return False
        if self.fitness_score != other.fitness_score:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def term(self):
        """Message field 'term'."""
        return self._term

    @term.setter
    def term(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'term' field must be of type 'int'"
            assert value >= 0 and value < 4294967296, \
                "The 'term' field must be an unsigned integer in [0, 4294967295]"
        self._term = value

    @builtins.property
    def candidate_id(self):
        """Message field 'candidate_id'."""
        return self._candidate_id

    @candidate_id.setter
    def candidate_id(self, value):
        if __debug__:
            assert \
                isinstance(value, str), \
                "The 'candidate_id' field must be of type 'str'"
        self._candidate_id = value

    @builtins.property
    def voter_id(self):
        """Message field 'voter_id'."""
        return self._voter_id

    @voter_id.setter
    def voter_id(self, value):
        if __debug__:
            assert \
                isinstance(value, str), \
                "The 'voter_id' field must be of type 'str'"
        self._voter_id = value

    @builtins.property
    def fitness_score(self):
        """Message field 'fitness_score'."""
        return self._fitness_score

    @fitness_score.setter
    def fitness_score(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'fitness_score' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'fitness_score' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._fitness_score = value
