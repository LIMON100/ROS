  ...
  void timer_callback()
  {
    ...
    if (!global_mission_poses_.empty()) {
        double d_to_wp = std::hypot(
            ...
            latest_odom_.pose.pose.position.y - global_mission_poses_[0].pose.position.y);
        
        if (d_to_wp < 1.5) { // Threshold for "Reached"
            RCLCPP_INFO(this->get_logger(), "Goal Cleared from Swarm List. Moving to next.");
            global_mission_poses_.erase(global_mission_poses_.begin());
        }
    }
