...
public:
  LeadershipManager() : Node("leadership_manager") {
    ...
    // pub_virtual_leader_ = this->create_publisher<skyhunter_msgs::msg::LeaderState>(
    //     "/swarm/virtual_leader/state", 10);

    // // Subscribe to "local" leader state (published by this robot's leader_node)
    // sub_local_leader_ = this->create_subscription<skyhunter_msgs::msg::LeaderState>(
    //     "leader_state", 10, [this](const skyhunter_msgs::msg::LeaderState::SharedPtr msg) {
    //         // ONLY republish if I am the elected LEADER
    //         if (this->current_role_ == RobotState::LEADER) {
    //             this->pub_virtual_leader_->publish(*msg);
    //         }
    //     });
  ...
  void logic_loop() {
    ...
    if (current_role_ == RobotState::CANDIDATE) {
        ...
        if ((now - takeover_start_time).seconds() >= 5.0) {
            ...
            // NOTE: We do NOT need to system() call activate if Nav2 was launched with autostart=true.
            // But we DO need to ensure our local leader_node is getting data.
        }
    }
