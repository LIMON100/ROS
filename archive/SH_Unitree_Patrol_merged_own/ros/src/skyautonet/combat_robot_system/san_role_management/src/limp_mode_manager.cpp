...
void LimpModeManager::wireInterfaces() {
    ...
    hub_sub_ = create_subscription<HubAnn>(
        ...
        std::bind(&LimpModeManager::onHubAnnouncement, this,
                  std::placeholders::_1),
        sub_opts);
    status_sub_ = create_subscription<Status>(
        "/swarm/robot_status", 10,
        std::bind(&LimpModeManager::onRobotStatus, this,
                  std::placeholders::_1),
        sub_opts);
...
void LimpModeManager::watchdogTick() {
    ...
    if (!hub_alive && !deputy_alive) {
        // 7 s grace from the more recent loss.
        rclcpp::Time worst = t;
        if (last_hub_alive_.has_value() && *last_hub_alive_ < worst) {
            worst = *last_hub_alive_;
        }
        if (last_deputy_alive_.has_value() && *last_deputy_alive_ < worst) {
            worst = *last_deputy_alive_;
        }
        const auto since_worst_ms =
            (t - worst).nanoseconds() / 1'000'000;
        if (since_worst_ms >= limp_guard_ms_ && !in_limp_mode_.load()) {
            enterLimpMode();
        }
...
void LimpModeManager::enableMeshAuthForFire() {
    ...
    if (!client->wait_for_service(std::chrono::seconds(1))) {
        RCLCPP_WARN(get_logger(),
            "/gun_trigger/set_parameters unavailable - "
...
void LimpModeManager::redirectVideoToAndroidDirect() {
    ...
    if (!client->wait_for_service(std::chrono::seconds(1))) {
        RCLCPP_WARN(get_logger(),
            "/video_sender_node/set_parameters unavailable - "
