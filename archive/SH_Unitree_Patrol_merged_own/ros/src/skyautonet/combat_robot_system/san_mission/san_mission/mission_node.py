...
class MissionNode(Node):
    def __init__(self):
        ...
        self._pose_sub = self.create_subscription(
            Odometry, "odom", self._on_pose, reliable_qos)

        # --- ADD WAYPOINT SUBSCRIBER ---
        self._wp_sub = self.create_subscription(
            WaypointCommand, "/swarm/cmd/waypoint", self._on_waypoint, reliable_qos)
    ...
    def _on_pose(self, msg: Odometry) -> None:
        # Extract X, Y, Yaw from Odometry instead of PoseStamped
        ...
        with self._ctx.lock:
            self._ctx.pose_xy = (msg.pose.pose.position.x, msg.pose.pose.position.y)
            self._ctx.yaw_rad = yaw
    def _on_waypoint(self, msg: WaypointCommand) -> None:
        # Update the Behavior Tree context with the new waypoints
        with self._ctx.lock:
            self._ctx.waypoints = [(p.x, p.y) for p in msg.waypoints]
            self._ctx.current_wp_index = 0
            self.get_logger().info(f"+++ NEW MISSION RECEIVED: {len(self._ctx.waypoints)} waypoints +++")
