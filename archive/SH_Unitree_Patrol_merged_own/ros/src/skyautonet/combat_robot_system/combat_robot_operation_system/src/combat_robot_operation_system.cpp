...
void CombatRobotOperationSystem::declareAllParameters() {
    ...
    // safety.*
    declare_parameter<std::string>("safety.gun_trigger_permission",
                                    "DENIED");
    ...
    const std::string& mode_str)
{
    ...
    if (it == modeMap().end()) {
        RCLCPP_FATAL(get_logger(),
            ...
            "Allowed: production|demo|lab_test|bench|development",
            mode_str.c_str());
...
void CombatRobotOperationSystem::wirePublishers() {
    ...
    m_heartbeat_timer_ = create_wall_timer(
        period_ns, std::bind(&CombatRobotOperationSystem::onHeartbeat,
                              this));
}
