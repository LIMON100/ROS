"""
*** DEPRECATED in v1.5 — DCN-2026-002 D-007 (3-Tier toolchain) + D-008 (IPC unification). ***

This Python multiprocessing bootstrap is being replaced by:
    ros2 launch san_bringup squadron.launch.py
or (production):
    sudo systemctl start san-squadron

Migration status (Phase 2-E):
    - Turn 1  ✅ (this file): san_bringup package + squadron.launch.py
                              + san-squadron.service. main.py retained
                              as fallback during migration.
    - Turn 2-7    (in progress): HW adapter Python classes (UnitreeGo2,
                                 RTK GNSS, NTRIP, LTE Modem, IMU, LiDAR,
                                 IMX678 Camera, Thermal Camera) being
                                 replaced by Tier 1 C++ rclcpp::Node
                                 packages. Until each replacement passes
                                 colcon test, the Python adapter remains
                                 active via main.py for regression
                                 fallback.
    - Turn 8-13   (planned):    Tier 2/3 modules (Mission, Comm,
                                 Perception, BLE) replaced by rclpy
                                 nodes.
    - Turn 16-17  (planned):    This file moves to archive/v15_python_prototype/.
    - v1.5.1      (final):      Archive directory removed from git history.

This file MUST NOT be modified for new functionality. New work goes
into the appropriate Tier 1/2/3 ROS 2 package. See:
    * docs/04_Change_Management/SkyHunter_v1.5_DCN-2026-002_2026-05-12.md
    * docs/05_Supplementary/ADR-006-ipc-unification-strategy.md
    * docs/04_Change_Management/Phase2E_baseline_2026-05-12.md
...
def _publish_operation_state(queues, cfg, stop: threading.Event):
    """1 Hz heartbeat carrying SAN v1.3 §11 deployment mode + ID mapping.

    Re-built each tick so a live config swap (Config v2) would surface
    on the next beat. Stays single-threaded by design — only main
    publishes operation_state, so consumers can trust ordering.
    """
    while not stop.is_set():
        try:
            msg = build_operation_state(cfg)
            publish(queues.operation_state, msg)
        except Exception as e:        # heartbeat must never kill main
            logging.getLogger("op_state").warning(
                "operation_state publish failed: %s", e)
        stop.wait(1.0)
...
def main():
    ...
    parser.add_argument("--deployment-mode", default=None,
                        ...
                        help="Override system.deployment_mode (SAN v1.3 §11). "
                             "Loads the matching <base>.<mode>.yaml overlay. "
                             "'development' additionally requires the "
                             "DEVELOPER_AUTH_TOKEN env var.")
    ...
    cfg = Config.load(args.config, deployment_mode=args.deployment_mode)
    try:
        mode = DeploymentMode.parse(cfg.deployment_mode)
    except ValueError as e:
        log.error("FATAL: %s", e)
        return 2
    # Development mode is gated by DEVELOPER_AUTH_TOKEN to prevent the
    # safety-relaxed harness from being entered on a real robot boot.
    try:
        validate_developer_auth(mode)
    except DeveloperAuthError as e:
        log.error("FATAL: %s", e)
        return 2
    if mode is DeploymentMode.DEVELOPMENT:
        emit_development_banner(log)
    log.info(f"config loaded from {args.config}; deployment_mode={mode.value}; "
             f"log_dir={log_dir} crash_dir={crash_dir} audit_dir={audit_dir} "
    ...
    op_state_stop = threading.Event()
    op_state_thread = None
    try:
        ...
        op_state_thread = threading.Thread(
            target=_publish_operation_state,
            args=(queues, cfg, op_state_stop),
            name="OpStatePub", daemon=True,
        )
        op_state_thread.start()
        ...
        if op_state_thread is not None:
            op_state_thread.join(timeout=1.0)
        lidar_shm.teardown()
        camera_shm.teardown()
