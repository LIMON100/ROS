...
public:
  // ─── Audit findings A1 + A5 test seams ──────────────────────────
  /// Inject the "this robot is leader" state from an OperationState
  /// (or test) without spinning a subscriber. Tracks the value the
  /// Gate-1 EmergencyStop SCOPE_LEADER_ONLY filter reads.
  void setLeaderRoleForTest(bool is_leader)
  {
    is_leader_role_.store(is_leader);
  }
  bool isLeaderRoleForTest() const {return is_leader_role_.load();}
...
private:
  ...
  CommandEcho echo_;
  int robot_id_ = 1;
  // ─── Audit finding A5 (P1) fix ──────────────────────────────────
  // Authoritative "I am the leader" flag, set by onOperationState
  // (and by `setLeaderRoleForTest`). Pre-fix Gate-1 EmergencyStop
  // SCOPE_LEADER_ONLY filter used `robot_id_ == 1` as a leader
  // proxy — wrong whenever the elected leader != robot_id 1.
  // Atomic for safe concurrent read from the EStop callback.
  std::atomic<bool> is_leader_role_{false};
  ...
  //   * /gate1/start_demo (std_srvs/Trigger)  — operator entry point
  //   * /gate1/demo_status (combat_robot_msgs/OperationState, 5s)  — status
  //   * /emergency_stop subscriber → RTH fallback
  //   * 5-sec watchdog tick → RTH if no phase advance in 60s
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr gate1_start_srv_;
  rclcpp::Publisher<combat_robot_msgs::msg::OperationState>::SharedPtr
    demo_status_pub_;
  rclcpp::Subscription<EStop>::SharedPtr gate1_estop_sub_;
  // Audit A5 — listen to /swarm/operation_state so is_leader_role_
  // tracks the elected leader (not the wrong robot_id==1 proxy).
  rclcpp::Subscription<combat_robot_msgs::msg::OperationState>::SharedPtr
    op_state_sub_for_leader_;
  rclcpp::TimerBase::SharedPtr demo_status_timer_;
  rclcpp::TimerBase::SharedPtr demo_watchdog_timer_;
  rclcpp_action::Client<combat_robot_msgs::action::ReturnToHome>::SharedPtr
    rth_client_;
  std::atomic<uint64_t> last_phase_advance_ms_{0};

  void onGate1StartDemo(
    const std_srvs::srv::Trigger::Request::SharedPtr req,
    std_srvs::srv::Trigger::Response::SharedPtr res);
  void onGate1EmergencyStop(EStop::SharedPtr msg);
  void publishDemoStatus();
  void demoWatchdogTick();
  void triggerRth(const std::string & reason);
  void onDemoPhaseTransition(DemoPhase next);
