...
{
  setGeometry(
    DEFAULT_GRID_CELLS, DEFAULT_GRID_CELLS,
    DEFAULT_RESOLUTION_M);
}

void CostMapPublisher::setGeometry(
  ...
  float origin_x, float origin_y)
{
  ...
  obstacle_.setGeometry(
    width, height, resolution_m,
    origin_x, origin_y);
  traversability_.setGeometry(
    width, height, resolution_m,
    origin_x, origin_y);
...
CostMapPublisher::buildMessage(uint32_t seq) const
{
  ...
  uint32_t inflated = 0;
  for (auto c : master_) {
    if (c == COST_LETHAL) {++lethal;} else if (c >= COST_WARN_LOW && c < COST_LETHAL) {++inflated;}
  }
