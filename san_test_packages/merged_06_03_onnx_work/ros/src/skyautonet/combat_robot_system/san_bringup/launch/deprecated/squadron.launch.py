...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # 'auto' = derive from robot_role / deployment_mode at the
        # IfCondition site (see hub_only / regression_only above).
        # CLI override: hub_features:=true|false / include_regression:=true|false.
        ...
        # Push every node into a robot-id namespace for multi-robot
        # cohabitation (simulation, paired bench tests). Each node
        # publishes/subscribes using relative topic names so this works
        # without code changes.
        PushRosNamespace(PythonExpression(
            ["'robot_' + str('", robot_id, "')"])),
