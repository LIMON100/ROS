...
private:
  ...
  /// Phase 1: latched advertisement of whether this driver is in
  /// STUB mode. Downstream consumers (perception, fire_authorization,
  /// mission) subscribe with `transient_local` to learn at startup
  ...
  /// When true (default), real-mode payloads with no model-specific
  /// decoder are DROPPED rather than republished as static-platform
  /// stub samples. Setting false re-enables the legacy publish-stub-
  /// on-undecoded-real behavior (only safe for early bringup).
  bool require_decoded_payload_;
  ...
  std::atomic<uint32_t> undecoded_drop_count_{0};   // real frames w/o decoder
};
