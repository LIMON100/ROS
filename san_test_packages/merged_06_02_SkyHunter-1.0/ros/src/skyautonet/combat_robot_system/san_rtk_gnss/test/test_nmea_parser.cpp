...
TEST(NmeaChecksum, N1_ChecksumMatch) {
  // Real u-blox sample:
  // "$GNGGA,121234.00,3733.5300,N,12658.7800,E,4,12,0.8,55.3,M,18.5,M,2.0,0001*HH"
...
// ─── Phase 0 PR-B: NaN / Inf / out-of-bounds rejection ──────────────────
// Pre-patch strtod parsed "nan" / "inf" silently; the parseDmToDeg
// downstream consumer would propagate NaN into lat/lon, poisoning
// SLAM and localization. Also: deg integer was unbounded, so
// "359912.0" → 3599° passed through.

TEST(DmToDeg, NaNTokenRejected) {
  EXPECT_FALSE(parseDmToDeg("nan", "N").has_value());
  EXPECT_FALSE(parseDmToDeg("NaN", "E").has_value());
  EXPECT_FALSE(parseDmToDeg("inf", "N").has_value());
  EXPECT_FALSE(parseDmToDeg("3700.NaN", "N").has_value());
  // Reasonable-looking string with NaN in minutes part is rejected
  // by toDouble; "3700.inf" likewise.
  EXPECT_FALSE(parseDmToDeg("3700.inf", "N").has_value());
}

TEST(DmToDeg, OutOfRangeDegreesRejected) {
  // 999° 12.34' — receiver glitch — must be refused.
  EXPECT_FALSE(parseDmToDeg("99912.34", "N").has_value());
  // 359° 59.99' — bounded-out at the parser level (deg > 180).
  EXPECT_FALSE(parseDmToDeg("35959.99", "E").has_value());
}

TEST(NmeaParseGga, GgaWithOutOfBoundsLatRejected) {
  // Hand-craft a GGA with deg=89 min=99.0 (which fails the min<60
  // check) — produces nullopt from parseDmToDeg → parseGga returns
  // nullopt.
  auto sent = "$GNGGA,121234.00,8999.0000,N,12658.7800,E,4,12,0.8,55.3,M,18.5,M,,";
  // Build checksum
  std::string body(sent + 1);  // strip leading '$'
  body.resize(body.size());
  uint8_t cs = 0;
  for (char c : body) {
    cs ^= static_cast<uint8_t>(c);
  }
  char hex[3];
  std::snprintf(hex, sizeof(hex), "%02X", cs);
  const std::string full = std::string("$") + body + "*" + hex;
  EXPECT_FALSE(parseGga(full).has_value());
}
