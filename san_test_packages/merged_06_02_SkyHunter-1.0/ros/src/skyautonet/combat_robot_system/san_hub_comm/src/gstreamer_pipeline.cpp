...
bool GStreamerPipeline::build(const std::string & description)
{
  ...
  if (p == nullptr) {
    RCLCPP_ERROR(
      logger_, "gst_parse_launch failed: %s",
...
GstBusSyncReply GStreamerPipeline::onBusMessageTrampoline(
  GstBus * bus,
  GstMessage * msg,
  gpointer user)
{
...
GstBusSyncReply GStreamerPipeline::onBusMessage(
  GstBus *,
  GstMessage * msg)
{
...
void GStreamerPipeline::fireEvent(
  PipelineEvent ev,
