...
bool ScenarioRunner::isLeaderPromotedBy(
...
bool ScenarioRunner::isLeaderPromotedAnyPriority(
  const LeaderRoleAnn & m,
  uint8_t priority)
{
  return m.role == LeaderRoleAnn::LEADER_PROMOTED &&
...
bool ScenarioRunner::isHubPromotedWithFullTakeover(
  const HubRoleAnn & m,
  uint32_t robot_id)
{
...
bool ScenarioRunner::isLimpModeAlert(
