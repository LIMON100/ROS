...
def generate_launch_description():
    ...
    # If namespace is "" → plain frame names (leader).
    # If namespace is "robot_2" → "robot_2/odom" etc. (follower).
    ...
    # Inject the namespaced frame names into ekf.yaml so the EKF
    # uses robot_2/odom + robot_2/base_footprint.
    configured_ekf = RewrittenYaml(
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            output="screen",
            parameters=[configured_ekf, {"use_sim_time": use_sim_time}],
        ...
        # 4. Lifecycle manager for map_server
        Node(
            package="nav2_lifecycle_manager",
            executable="lifecycle_manager",
            name="lifecycle_manager_localization",
        ...
        # 5. Static map → <ns>/odom transform (the key fix for followers)
        # Node(
        #     package="tf2_ros",
