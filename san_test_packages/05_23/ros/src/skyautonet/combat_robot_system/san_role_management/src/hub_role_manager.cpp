...
void HubRoleManager::declareParameters() {
    ...
    declare_parameter<int>("hub_heartbeat_timeout_ms",
...
void HubRoleManager::readParameters() {
    ...
    lifecycle_service_wait_ms_ = std::max(0,
...
void HubRoleManager::wireInterfaces() {
    ...
    announce_sub_ = create_subscription<HubAnn>(
        "/swarm/hub/role_announce", qos,
        std::bind(&HubRoleManager::onHubAnnouncement, this,
                  std::placeholders::_1),
        sub_opts);
    robot_status_sub_ = create_subscription<Status>(
        "/swarm/robot_status", 10,
        std::bind(&HubRoleManager::onRobotStatus, this,
                  std::placeholders::_1),
        sub_opts);
...
void HubRoleManager::injectAuditSampleForTest(uint32_t robot_id,
                                               int64_t ms_ago,
                                               bool sbc1, bool sbc2,
...
void HubRoleManager::injectHubStatusForTest(uint32_t hub_id, uint64_t /*now_ms*/,
                                             bool sbc1, bool sbc2)
{
...
void HubRoleManager::watchdogTick() {
    ...
    if (hub_silent || hub_unhealthy) {
        ...
        startHubPromotionAsync(hub_silent
