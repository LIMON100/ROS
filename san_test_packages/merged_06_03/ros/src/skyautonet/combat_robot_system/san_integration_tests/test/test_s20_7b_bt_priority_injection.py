...
def generate_test_description():
    ...
    return launch.LaunchDescription([
        ...
        launch.actions.TimerAction(
            # mission_node Python import + rclpy spin-up takes ~3s on
            # native Linux but can stretch past 5s when run from
            # /mnt/c on WSL (cold-cache Python imports off a Windows
            # mount). 6s gives both environments a comfortable margin.
            period=6.0,
...
class TestBtPriorityInjection(unittest.TestCase):
    ...
    def _await_discovery(self, timeout_s: float = 8.0):
        """Block until mission_node has discovered our injection
        publishers and our subscription has found mission_node's
        /cmd_vel publisher. Without this, the first inject_emergency()
        races DDS discovery — the EmergencyStop is published before
        mission_node has subscribed, and (since the QoS is VOLATILE)
        it is dropped on the floor."""
        end = time.monotonic() + timeout_s
        while time.monotonic() < end:
            rclpy.spin_once(self.probe, timeout_sec=0.05)
            estop_seen = self.probe.estop_pub.get_subscription_count() > 0
            manual_seen = self.probe.manual_pub.get_subscription_count() > 0
            if estop_seen and manual_seen:
                # Brief tail-spin so mission_node's cmd_vel publisher
                # can also be discovered by our subscription.
                self._spin_for(0.3)
                return
        raise AssertionError(
            "DDS discovery timed out: mission_node did not subscribe "
            "to /robot_3/emergency_stop or /robot_3/manual_override "
    ...
    def test_priority_emergency_then_manual_then_release(self):
        # Block until DDS discovery is complete on both directions
        # (probe→mission_node injections AND mission_node→probe
        # /cmd_vel). Replaces the bare _spin_for(1.0) settle, which
        # raced discovery on slow file-systems and dropped the first
        # EmergencyStop under VOLATILE QoS.
        self._await_discovery(timeout_s=10.0)
