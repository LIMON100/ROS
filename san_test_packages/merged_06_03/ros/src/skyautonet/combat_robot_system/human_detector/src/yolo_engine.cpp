...
std::vector<YOLOBox> YoloEngine::run_inference(cv::Mat& frame) {
    ...
    std::cerr << "[YOLO] output dims =";
    for (auto s : shape) std::cerr << " " << s;
    std::cerr << std::endl;


    // One-time visibility into the real output shape.
    static bool printed = false;
    if (!printed) {
      std::cerr << "[YOLO] output dims = ";
      for (auto s : shape) std::cerr << s << " ";
      std::cerr << std::endl; printed = true;
    }

    // Collapse to the two meaningful dims (YOLOv8: 84 x 8400, either order).
    std::vector<int64_t> dims;
    for (auto s : shape) if (s > 1) dims.push_back(s);
    if (dims.size() != 2) { return {}; }            // guard: unexpected output
    int d0 = (int)dims[0], d1 = (int)dims[1];
    int num_attrs = std::min(d0, d1);               // 4 + num_classes  (e.g. 84)
    int num_boxes = std::max(d0, d1);               // anchors          (e.g. 8400)
    (void)num_boxes;

    cv::Mat raw(d0, d1, CV_32F, data);
    cv::Mat det = (d0 == num_attrs) ? raw.t() : raw; // -> [boxes, attrs]
    ...
    for (int i = 0; i < det.rows; ++i) {
      ...
      cv::Point cid; double maxScore;
      cv::minMaxLoc(scores, 0, &maxScore, 0, &cid);
      if (maxScore > 0.45) {
        float w = row[2]*x_factor, h = row[3]*y_factor;
