...
class Coordinator(Node):
    def __init__(self):
        ...
        self.lvel = (0.0, 0.0) 
        self.trail = []       
        self.gap_behind = {3: 3.0, 4: 6.0, 5: 9.0, 2: 12.0}
        self._llast = None
        self.ugv = {}
        self.last_w = 0.0
        self.wp_idx = 1
    ...
    def on_leader(self, m):
        ...
        if not self.trail or math.hypot(
                x - self.trail[-1][0], y - self.trail[-1][1]) > 0.15:
    ...
    def broker(self):
        self.pub_obstacles()  # 장애물 맵 @2Hz 브로드캐스트(Leader→UGV)
        for n in CHAIN:
            p = PRED[n]
            if p == 0:
                if self.leader is None:
    ...
    def pub_ref_paths(self):
        # B3: per-robot reference = the leader's TRAVELED trail, truncated to end
        # gap_behind[n] metres behind the leader -> nav2 FollowPath tracks it and
        # holds the convoy gap (instead of racing the full plan to the end).
        if len(self.trail) < 2:
            return
        for n in CHAIN:
            gap = self.gap_behind.get(n, 3.0)
            d, i = 0.0, len(self.trail) - 1
            while i > 0:
                seg = math.hypot(self.trail[i][0] - self.trail[i - 1][0],
            ...
            for px, py in pts:
                ...
                ps.pose.orientation.w = 1.0
                path.poses.append(ps) 
            self.ref_path_pub[n].publish(path)
