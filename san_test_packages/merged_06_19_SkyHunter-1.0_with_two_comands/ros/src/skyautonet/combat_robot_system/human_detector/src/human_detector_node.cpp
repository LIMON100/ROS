    ...
    det.has_thermal_signature = false;
    {
      ...
      if (fresh) {
        ...
        if (x2 > x1 && y2 > y1) {
          ...
          double mn, mx;
          cv::minMaxLoc(patch, &mn, &mx);
          double mean = cv::mean(patch)[0];
          det.thermal_avg_temp_c =
            static_cast<float>(mean * thermal_scale_ + thermal_offset_);
          det.thermal_max_temp_c =
            static_cast<float>(mx * thermal_scale_ + thermal_offset_);
