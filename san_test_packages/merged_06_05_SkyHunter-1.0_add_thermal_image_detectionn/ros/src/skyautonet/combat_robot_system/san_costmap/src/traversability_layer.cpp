...
void TraversabilityLayer::setGeometry(
  int width, int height,
  float resolution_m,
...
void TraversabilityLayer::rasterizeGround()
{
  ...
  for (const auto & p : ground_->points) {
    // std::floor (not truncation): negative-edge points map to cell
    // -1 (skipped) rather than folding into cell 0.
