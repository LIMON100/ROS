...
TEST_F(HandlePublishingTest, StartProducesEncryptedSrtUri) {
  ...
  // HC1/HC2 security patch: passphrase is NOT embedded in the URI
  // (which can leak via logs/metrics). It travels in the separate
  // handle.passphrase field, protected by the operator-topic policy
  // (DDS partition / authenticated bridge). Positively assert that
  // the URI contains neither so a regression that re-embeds the
  // secret would fail this test.
  EXPECT_EQ(h.srt_uri.find("passphrase="), std::string::npos)
    << "Passphrase must not leak into srt_uri (HC1/HC2). URI was: "
    << h.srt_uri;
  EXPECT_EQ(h.srt_uri.find("pbkeylen="), std::string::npos)
    << "Key-length param must not appear in srt_uri (HC1/HC2)";
