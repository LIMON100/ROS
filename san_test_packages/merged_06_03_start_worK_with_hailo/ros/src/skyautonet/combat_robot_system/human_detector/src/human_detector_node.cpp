  ...
  sensor_msgs::msg::CompressedImage::SharedPtr msg)
{
  ...
  try {
    ...
    // --- CRITICAL FIX: Catch OpenCV crashes here too ---
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
  ...
  sensor_msgs::msg::Image::SharedPtr msg)
{
  ...
  try {
    ...
    // --- CRITICAL FIX: Catch OpenCV allocation crashes ---
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
...
//   // =================================================================
//   // Drop empty frames or frames with impossible dimensions (prevents 54TB crash)
//   // =================================================================
...
//     // Catch ANY OpenCV/Bridge crash and survive!
//     RCLCPP_WARN_THROTTLE(
//       get_logger(), *get_clock(), 5000,
//       "OpenCV/Bridge Exception (ignoring frame): %s", e.what());
//     ++frames_dropped_;
//   }
// }
  ...
  const std_msgs::msg::Header & src_header)
{
  ...
  //   int n_person = 0, n_vehicle = 0, n_drone = 0, n_weapon = 0, n_animal = 0, n_other = 0;
  //   for (const auto & det : out->detections) {
  //     switch (det.class_id) {
  //       case Detection_msg::CLASS_PERSON:  ++n_person;  break;
  //       case Detection_msg::CLASS_VEHICLE: ++n_vehicle; break;
  //       case Detection_msg::CLASS_DRONE:   ++n_drone;   break;
  //       case Detection_msg::CLASS_WEAPON:  ++n_weapon;  break;
  //       case Detection_msg::CLASS_ANIMAL:  ++n_animal;  break;
  //       default:                           ++n_other;   break;
  //     } 
  //   }
  //   RCLCPP_INFO_THROTTLE(
  //     get_logger(), *get_clock(), 1000,
  //     "DETECTED  person=%d vehicle=%d drone=%d weapon=%d animal=%d other=%d  (total=%zu)",
  //     n_person, n_vehicle, n_drone, n_weapon, n_animal, n_other,
  //     out->detections.size());
  // }
