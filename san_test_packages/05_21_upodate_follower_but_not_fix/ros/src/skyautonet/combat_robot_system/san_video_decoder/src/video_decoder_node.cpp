...
bool VideoDecoderNode::buildPipeline() {
    ...
    if (prefer_hw_ && have_mpp) {
        ...
        // do-timestamp=true makes appsrc stamp incoming buffers with
        // the running clock (we don't have a meaningful PTS in
        // CompressedImage).
        desc =
    ...
    GstStateChangeReturn rc = gst_element_set_state(pipeline_,
                                                     GST_STATE_PLAYING);
...
void VideoDecoderNode::wireInterfaces() {
    ...
    compressed_sub_ = create_subscription<sensor_msgs::msg::CompressedImage>(
        ...
        std::bind(&VideoDecoderNode::onCompressedImage, this,
                  std::placeholders::_1),
        sub_opts);
    ...
    sensor_msgs::msg::CompressedImage::SharedPtr msg)
{
    ...
    GstBuffer* buf = gst_buffer_new_allocate(nullptr, msg->data.size(),
                                              nullptr);
    if (!buf) {
        ++appsrc_push_errors_;
        ++frames_dropped_;
...
void VideoDecoderNode::onDecodedSample(GstSample* sample,
    ...
    const std::vector<uint8_t>& h265_packet, int timeout_ms)
{
    ...
    GstBuffer* buf = gst_buffer_new_allocate(nullptr, h265_packet.size(),
                                              nullptr);
