...
// ─── R9: Maximum 10-bit length payload (1023 bytes) ────────────────────
//
// The RTCM3 length field is 10 bits → max payload 1023 bytes. A bug
// in the length-mask (e.g. accidentally 0x1FF, 9-bit) would silently
// reject the largest legal frames. This regression-guards the
// payload.size() & 0x3FF mask in buildFrame() AND the parser's
// matching extraction.
TEST(Rtcm3, R9_MaxLengthPayload1023Bytes) {
  std::vector<uint8_t> payload(1023, 0xAA);
  // Encode message_type 1077 (MSM7 GPS) = 0x435 in first 12 bits
  // payload[0] high 8 bits = 0x43, payload[1] high 4 bits = 0x5
  payload[0] = 0x43;
  payload[1] = 0x50;
  auto buf = buildFrame(payload);
  ASSERT_EQ(buf.size(), 3u + 1023u + 3u);   // header + payload + CRC

  size_t consumed = 0;
  auto f = parseRtcm3(buf, &consumed);
  ASSERT_TRUE(f.has_value()) << "max-length frame must parse";
  EXPECT_EQ(consumed, 1029u);
  EXPECT_EQ(f->message_type, 1077u);
  EXPECT_EQ(f->bytes.size(), 1029u);
}

// ─── R10: Two consecutive frames parse sequentially without overlap ───
//
// Driver loops on parseRtcm3() to drain the socket. This test
// verifies that consuming exactly the first frame leaves the second
// frame at the buffer head, ready for the next call — off-by-one
// regression guard in the consumed= calculation.
TEST(Rtcm3, R10_TwoConsecutiveFramesParseSequentially) {
  auto frame1 = buildFrame({0x12, 0x34});   // msg_type from these bits
  auto frame2 = buildFrame({0x3E, 0xC0});   // 1004
  std::vector<uint8_t> buf;
  buf.insert(buf.end(), frame1.begin(), frame1.end());
  buf.insert(buf.end(), frame2.begin(), frame2.end());

  size_t c1 = 0;
  auto r1 = parseRtcm3(buf, &c1);
  ASSERT_TRUE(r1.has_value());
  EXPECT_EQ(c1, frame1.size())
    << "first parse must consume exactly frame1 (no over/under-read)";

  buf.erase(buf.begin(), buf.begin() + c1);
  size_t c2 = 0;
  auto r2 = parseRtcm3(buf, &c2);
  ASSERT_TRUE(r2.has_value());
  EXPECT_EQ(c2, frame2.size());
  EXPECT_EQ(r2->message_type, 1004u);
}
