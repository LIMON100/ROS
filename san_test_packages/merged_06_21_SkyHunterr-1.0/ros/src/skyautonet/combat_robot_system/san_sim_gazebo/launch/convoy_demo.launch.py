...
def _convoy_nodes(gap, ugv_vmax, leader_vmax, leader_wmax, lidar_only, nav2_ids):
    bridge_args = []
    for n in CHAIN:
        bridge_args += [
    ...
    NAV2_FOLLOWERS = [int(x) for x in nav2_ids.split(",") if x.strip()]
    for n in CHAIN:
        nodes.append(Node(
            package="tf2_ros", executable="static_transform_publisher",
            name=f"map_to_odom_{n}",
            arguments=[("0" if n in NAV2_FOLLOWERS else str(SPAWN_X[n])),
                        "0", "0", "0", "0", "0", "map", f"robot_{n}/odom"],
            output="screen")) 
...
def _bringup(context, *args, **kwargs):
    ...
    nav2_ids = LaunchConfiguration("nav2_followers").perform(context).strip()
    if nav2_ids == "all":
        nav2_ids = ",".join(str(n) for n in CHAIN)
    actions = _spawn_nodes(world) + _convoy_nodes(
        gap, ugv_vmax, leader_vmax, leader_wmax, lidar_only, nav2_ids
    )   
    out = [TimerAction(period=warmup, actions=actions)]
    if nav2_ids:
        nav2_launch = os.path.join(
            get_package_share_directory("san_nav2"), "launch", "convoy_nav2.launch.py")
        out.append(TimerAction(period=warmup + 15.0, actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(nav2_launch),
                launch_arguments={"robot_ids": nav2_ids}.items())]))
    return out
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "nav2_followers",
                default_value="",
                description="comma list of UGV ids running their OWN nav2 "
                "(skip breadcrumb + auto-include convoy_nav2). "
                "'' = all breadcrumb, 'all' = every UGV (heavy on one PC).",
            ),
            gz_resource,
            sitl,
            viz,
