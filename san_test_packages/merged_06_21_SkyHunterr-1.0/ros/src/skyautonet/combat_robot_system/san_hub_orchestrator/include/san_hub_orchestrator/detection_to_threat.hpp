...
/// Real-world height prior [m] for monocular ranging of a Detection class.
/// Returns 0 when the class has no stable prior — range stays unknown.
double classToRealHeightM(uint8_t class_id);

/// Geolocate a detection: pixel bbox centre (px,py) + principal point
/// (cx0,cy0) + focal length → camera-relative angles, composed with robot
/// yaw + gimbal pan/tilt [rad] into a world-frame bearing/elevation [deg].
/// Range comes from depth_m when > 0, else from the pinhole height prior.
