...
{
  std::vector<SlotXY> out;
  out.reserve(n_followers);
  for (std::size_t i = 0; i < n_followers; ++i) {
    const float ang = 2.0f * static_cast<float>(M_PI) *
      static_cast<float>(i) / static_cast<float>(n_followers);
    out.push_back({radius_m * std::cos(ang), radius_m * std::sin(ang)});
  }
  return out;
}
