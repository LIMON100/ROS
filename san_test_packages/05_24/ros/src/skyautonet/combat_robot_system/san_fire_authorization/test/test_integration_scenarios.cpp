...
AuthMessage makeAuthMsg(uint32_t request_id,
                        uint8_t cmd,
                        uint64_t nonce,
                        uint64_t ts_ms,
...
AuditEntry baseAuditEntry(const AuthMessage& am,
                          uint64_t now_ms,
                          uint32_t hub_term = 2,
...
GateDecision runGate(HmacAuthenticator& hmac,
...
std::size_t countLinesContaining(const std::string& path,
...
TEST(IntegrationScenarios, S4_Key1ThenKey2AfterTimeoutDenied) {
  ...
  EXPECT_EQ(countLinesContaining(audit_path, "\"reason\":\"TWO_KEY_TIMEOUT\""),
            1u);
...
TEST(IntegrationScenarios, S5_NonceReplayDeniedAndAudited) {
  ...
  EXPECT_EQ(countLinesContaining(audit_path, "\"reason\":\"NONCE_REUSE\""),
            1u);
