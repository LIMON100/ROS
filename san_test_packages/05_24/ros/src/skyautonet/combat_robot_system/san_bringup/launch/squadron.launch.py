...
def generate_launch_description():
    ...
    follower_only = GroupAction(
        ...
        {"robot_id": ParameterValue(robot_id, value_type=int)},
                ...
                remappings=[
                    ("~/cmd_vel", "cmd_vel_nav"), # <-- FIXED: Must be cmd_vel_nav
                    ("/leader_state", "/leader_state"),
                    ("/tf", "/tf"),               # <-- FIXED: Force global TF
                    ("/tf_static", "/tf_static")  # <-- FIXED: Force global TF
                ],
            ),
        ]
    )
    ...
    # 2. Nodes for the Leader (Global Namespace)
    global_nodes = GroupAction(
        condition=IfCondition(PythonExpression(["'", robot_id, "' == '1'"])),
        actions=[
            always_on, hub_only, leader_only, follower_only, regression_only
    ...
    # 3. CRITICAL FIX: Nav2 and Localization handled OUTSIDE of PushRosNamespace to prevent /robot_N/robot_N/ crashes
    nav2_and_loc = GroupAction(
        actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(loc_share, "launch", "localization.launch.py")),
                launch_arguments={
                    "use_sim_time": "true",
                    "namespace": PythonExpression(["'robot_' + str('", robot_id, "') if '", robot_id, "' != '1' else ''"]),
                }.items(),
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(nav_share, "launch", "nav2.launch.py")),
                launch_arguments={
                    ...
                    "params_file": PythonExpression([
                        "'", os.path.join(nav_share, "config", "nav2_follower_params.yaml"), "' if '", robot_id, "' != '1' else '", os.path.join(nav_share, "config", "nav2_params.yaml"), "'"
                    ]),
                }.items(),
            )
        ]
