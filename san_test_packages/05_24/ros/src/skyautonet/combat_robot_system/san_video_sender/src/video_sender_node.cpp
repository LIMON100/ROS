...
void VideoSenderNode::wireInterfaces() {
    ...
    handle_sub_ = create_subscription<Handle>(
        "/video/handle", qos,
        std::bind(&VideoSenderNode::onHandle, this,
                  std::placeholders::_1));
}

void VideoSenderNode::qualityToResolution(uint8_t quality, int& w, int& h,
                                          int& bitrate_kbps)
{
...
void VideoSenderNode::onHandle(Handle::SharedPtr msg) {
    ...
    if (msg->status == Handle::STATUS_ERROR) {
        RCLCPP_ERROR(get_logger(), "Handle reported ERROR: %s",
                     msg->error_msg.c_str());
        return;
    }
...
std::string VideoSenderNode::buildDescription(int bitrate_kbps,
...
bool VideoSenderNode::startPipeline(int bitrate_kbps, int width, int height) {
    ...
    if (err) {
        RCLCPP_WARN(get_logger(), "gst_parse_launch warning: %s",
                    err->message);
        g_error_free(err);
    }
