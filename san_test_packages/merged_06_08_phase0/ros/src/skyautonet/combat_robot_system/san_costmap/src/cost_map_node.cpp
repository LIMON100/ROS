...
void CostMapNode::declareParameters()
{
  ...
  // ★ v1.5.1 (DCN-2026-003 D-002, 2026-05-13): 1000 → 500 ms (1Hz → 2Hz)
  declare_parameter<int>("update_period_ms", 500);
...
void CostMapNode::runUpdate()
{
  ...
  // [DCN-2026-006 EXT D-016] Feed the obstacle cloud to the
  // traversability layer too, so obstacle-shadowed cells are excluded
  // from ditch detection. Without this the occlusion suppression is
  // inert in production (obstacle_count_grid_ stays all-zero).
  publisher_.traversabilityLayer().setObstacleCloud(latest_obstacle_);
