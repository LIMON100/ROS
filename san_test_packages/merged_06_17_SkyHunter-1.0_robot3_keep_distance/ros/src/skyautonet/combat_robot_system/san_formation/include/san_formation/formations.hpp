...
/// DCN-2026-026 C-2 — Encircle combat geometry: n_followers slots
/// evenly spaced (2π/n) on a ring of radius_m around the origin. The
/// origin is the THREAT anchor (world point) — not the leader; the
/// caller transforms with an anchor pose of yaw 0. The leader gets no
/// ring slot (it holds its own position during combat).
std::vector<SlotXY> encircleSlots(std::size_t n_followers, float radius_m);
