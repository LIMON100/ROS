...
/// DCN-2026-026 C-1 — multi-threat constants (shared with the node).
constexpr float kThreatMergeDeg = 15.0f;        // bearings within → 1 cluster
constexpr std::size_t kMaxThreatClusters = 2;   // concurrent threat cap

/// Merge raw threat bearings into ≤ max_clusters clusters (greedy in
/// input order; incremental circular mean per cluster; bearings beyond
/// the cap are dropped). Pure logic — standalone testable.
/// DCN-2026-026 C-1.
std::vector<float> clusterThreatBearings(
  const std::vector<float> & bearings_deg,
  float merge_deg = kThreatMergeDeg,
  std::size_t max_clusters = kMaxThreatClusters);
...
struct AllocatorInput
{
  ...
  /// DCN-2026-026 C-1 — multi-threat focus. Bearings (same frame as
  /// `output_frame`) are merged into ≤ kMaxThreatClusters clusters
  /// (kThreatMergeDeg). At most THREAT_FOLLOWER_COUNT(3) followers in
  /// total are re-pointed — single cluster keeps the legacy 3
  /// (SDD §8.6.1, TST A7/A10); with 2 clusters each takes ≤ 2 and the
  /// budget splits by proximity. Leader/Hub are never re-pointed.
  /// `threat_bearing_deg` above is folded into this list.
