...
: rclcpp::Node("rtk_gnss_node", opts), serial_(std::move(serial))
{
  ...
  // Phase 1: latched stub-status. Published after open() decides
  // whether the receiver is real.
  ...
  // Latch stub status: serial open failed (and we're in stub_on_no_serial)
  // means the linked backend is producing no data → it's a stub.
  std_msgs::msg::Bool stub_msg;
  stub_msg.data = !serial_->isOpen();
  stub_status_pub_->publish(stub_msg);

  RCLCPP_INFO(
    get_logger(),
    "RtkGnssNode UP: device=%s baud=%d frame=%s stub=%d",
    device_.c_str(), baud_, frame_id_.c_str(),
    static_cast<int>(stub_msg.data));
}
...
void RtkGnssNode::publishFix(const std::string & gga_line)
{
  ...
  // [LOC-1 follow-up] RTK-quality-scaled position covariance so the EKF
  // down-weights RTK_FLOAT instead of snapping to a noisy/jumpy fix.
  applyPositionCovariance(fix, static_cast<uint8_t>(r->fix_type));
  ...
  const rclcpp::Time & stamp)
{
  ...
  double yaw_var;
  if (fix_type == static_cast<uint8_t>(ft::RtkFix)) {
    yaw_var = 0.0017;
  } else if (fix_type == static_cast<uint8_t>(ft::RtkFloat)) {yaw_var = 0.030;} else {
    yaw_var = 1.0;
  }

  for (auto & v : m.orientation_covariance) {
    v = 0.0;
  }
...
// ─── applyPositionCovariance (LOC-1 follow-up) ──────────────────────────

void RtkGnssNode::applyPositionCovariance(
  sensor_msgs::msg::NavSatFix & fix, uint8_t fix_type)
{
  using ft = san_rtk_gnss::FixType;
  double sigma_xy;     // horizontal 1-σ, metres
  double sigma_z;      // vertical   1-σ, metres
  switch (static_cast<ft>(fix_type)) {
    case ft::RtkFix:
      sigma_xy = 0.02; sigma_z = 0.05; break;       // 2 cm
    case ft::RtkFloat:
      sigma_xy = 0.30; sigma_z = 0.50; break;       // 30 cm — down-weighted
    case ft::Dgps:
      sigma_xy = 1.0;  sigma_z = 2.0;  break;
    default:
      sigma_xy = 3.0;  sigma_z = 5.0;  break;        // autonomous / estimated
  }
  for (auto & c : fix.position_covariance) {
    c = 0.0;
  }
  fix.position_covariance[0] = sigma_xy * sigma_xy;     // E
  fix.position_covariance[4] = sigma_xy * sigma_xy;     // N
  fix.position_covariance[8] = sigma_z * sigma_z;       // U
...
void RtkGnssNode::onRtcm(const std_msgs::msg::UInt8MultiArray::SharedPtr msg)
{
  ...
  const size_t want = msg->data.size();
  const size_t got = serial_->write(msg->data);
  if (got != want) {
    // Short write — most commonly the stub backend returning 0 (no
    // real receiver), but also EAGAIN / pipe-full on real hardware.
    // Increment a separate "drop" counter and log throttled so
    // operators see RTCM is NOT actually flowing.
    ++rtcm_inject_drop_count_;
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
      "RTCM write short: requested=%zu wrote=%zu — receiver not "
      "actually receiving corrections (drop_count=%u)",
      want, got, rtcm_inject_drop_count_.load());
    return;
  }
  ++rtcm_inject_count_;
}
