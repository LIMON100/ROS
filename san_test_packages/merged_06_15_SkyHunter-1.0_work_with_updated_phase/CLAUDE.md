...
SkyHunter 1.0 is a ROS 2 Jazzy stack for autonomous swarm operation of an 8-robot
...
This repo is edited on Windows, but **the stack builds and runs on Ubuntu 24.04 +
ROS 2 Jazzy** — `colcon`/`ros2`/`source` commands below run on Linux (CI containers or
...
```bash
ros2 launch san_bringup squadron.launch.xml                      # 8-robot squadron
ros2 launch san_bringup squadron.launch.xml robot_id:=2 robot_role:=hub  # single robot
ros2 launch san_sim_gazebo swarm_sim.launch.py                   # Gazebo sim (8-robot swarm)
ros2 launch san_sim_gazebo convoy_demo.launch.py                 # Go2 SITL convoy demo (leader dog + 4 UGV)
```
...
holds `squadron.launch.xml` (the `.py` launch files are deprecated under
`san_bringup/launch/deprecated/` per DCN-2026-014).
...
Hub UGV runs dual RK3588 SBCs (Boardcon EM3588) via the multi-stage Docker stacks in
...
cross-build targets RK3588 / Boardcon EM3588 (`requirements-arm64.txt`).
