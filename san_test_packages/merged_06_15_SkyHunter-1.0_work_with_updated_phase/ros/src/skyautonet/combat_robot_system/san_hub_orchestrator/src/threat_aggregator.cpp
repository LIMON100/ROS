...
bool ThreatAggregator::ingest(const ThreatInput & in)
{
  ...
  if (it == slots_.end()) {
    ...
    s.agg.has_position = in.has_position;
    s.agg.bearing_deg = in.bearing_deg;
    s.agg.elevation_deg = in.elevation_deg;
    s.agg.range_m = in.range_m;
  ...
  // Latest positional fix wins (DCN-2026-026 C-2) — a folded re-report
  // with a position refreshes a slot created without one.
  if (in.has_position) {
    it->second.agg.has_position = true;
    it->second.agg.bearing_deg = in.bearing_deg;
    it->second.agg.elevation_deg = in.elevation_deg;
    it->second.agg.range_m = in.range_m;
  }
