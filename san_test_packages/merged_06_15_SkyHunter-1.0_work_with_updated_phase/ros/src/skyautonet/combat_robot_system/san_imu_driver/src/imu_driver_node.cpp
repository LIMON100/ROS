...
: rclcpp::Node("imu_driver_node", opts), serial_(std::move(serial))
{
  ...
  // Phase 1: stub status as a latched Bool. Consumers subscribing
  // with transient_local + depth 1 see the current value at startup.
  ...
  // Latch the stub status. transient_local QoS means late-joining
  // subscribers immediately receive the current value.
  std_msgs::msg::Bool stub_msg;
  stub_msg.data = stub_mode_;
...
void ImuDriverNode::declareParameters()
{
  ...
  declare_parameter<bool>("require_decoded_payload", true);
...
void ImuDriverNode::loadParameters()
{
  ...
  require_decoded_payload_ =
    get_parameter("require_decoded_payload").as_bool();
  sync_byte_0_ =
...
{
  // TODO(skyautonet) Turn 5.5: decode model-specific payload layout (Xsens MT,
  // VectorNav, Bosch BMI088, etc.).
  //
  // Until the decoder lands, the safe behavior is to DROP the frame
  // (with a throttled WARN + per-drop counter) rather than republish
  // the static-platform stub sample on every real frame — doing so
  // would poison the EKF with a stationary, level robot estimate
  // regardless of true motion.
  //
  // Legacy behavior (republish stub on undecoded real payload) is
  // available behind the `require_decoded_payload=false` parameter,
  // but only for bringup test rigs.
  if (require_decoded_payload_) {
    ++undecoded_drop_count_;
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
      "IMU real frames arriving (%u total) but model-specific "
      "decoder is not yet wired (TODO Turn 5.5) — DROPPING this "
      "frame to avoid publishing stub data on top of real motion. "
      "Set parameter require_decoded_payload=false to opt into the "
      "legacy stub-on-undecoded behavior (bringup only).",
      frame_count_.load());
    return;
  }
  RCLCPP_DEBUG_THROTTLE(
    ...
    "(require_decoded_payload=false → publishing stub sample)",
    payload.size());
  publishStubSample();
}
...
void ImuDriverNode::onHealthTick()
{
  RCLCPP_INFO(
    get_logger(),
    "imu mode=%s frames=%u drops=%u undecoded_drops=%u serial=%d",
    stub_mode_ ? "STUB" : "REAL",
    frame_count_.load(), drop_count_.load(),
    undecoded_drop_count_.load(),
