...
: rclcpp::Node("formation_node", opts)
{
  ...
  threat_sub_ = create_subscription<combat_robot_msgs::msg::ThreatAlert>(
    "/swarm/threat_alert_raw", rclcpp::QoS(10).reliable(),
    std::bind(&FormationNode::onThreatAlert, this, std::placeholders::_1));
...
void FormationNode::declareParameters()
{
  ...
  declare_parameter<double>("engage_range_m", 10.0);
  declare_parameter<double>("encircle_radius_m", 5.0);
...
void FormationNode::loadParameters()
{
  ...
  engage_range_m_ = get_parameter("engage_range_m").as_double();
  encircle_radius_m_ = get_parameter("encircle_radius_m").as_double();
}
...
{ 
  ...
  if (msg->has_position && msg->range_m > 0.0f &&
    msg->range_m < static_cast<float>(engage_range_m_))
  { 
    auto it = robot_poses_.find(leader_robot_id_);
    if (it == robot_poses_.end()) {combat_active_ = false; return;}
    const float b = msg->bearing_deg * static_cast<float>(M_PI) / 180.0f;
    target_world_.x = it->second.pose.x + msg->range_m * std::cos(b);
    target_world_.y = it->second.pose.y + msg->range_m * std::sin(b);
    combat_active_ = true;
  } else {
...
void FormationNode::onAssignTick()
{
  PlanResult plan;
  {
    ...
    if (current_assignment_.empty()) {
      ...
      if (leader_it == robot_poses_.end()) {return;}
      const auto & leader = leader_it->second.pose;
...
FormationNode::PlanResult FormationNode::recomputeAssignmentLocked()
{
  ...
  if (n == 0) {
    result.error_reason = "no robots known";
  ...
  // const PoseXY leader_world = leader_it->second.pose;

  // // Generate slots in LEADER LOCAL frame.
  // auto slots_local = generateSlots(
  //   current_formation_, n, spacing_d_m_, spread_theta_deg_);

  const PoseXY leader_world = leader_it->second.pose;
    
  PoseXY anchor_world = leader_world;
  Formation form = current_formation_;
  float d_use = spacing_d_m_;
  if (combat_active_) {
    anchor_world.x = target_world_.x;
    anchor_world.y = target_world_.y;
    anchor_world.yaw = std::atan2(
      leader_world.y - target_world_.y, leader_world.x - target_world_.x);
    form = Formation::Encircle;
    d_use = static_cast<float>(encircle_radius_m_);
  } 
  last_anchor_ = anchor_world;
  
  auto slots_local = generateSlots(
    form, n, d_use, spread_theta_deg_);
  ...
  // auto cost = buildCostMatrix(robots_world, slots_local, leader_world);
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    s.slot_type = combat_active_ ? (j == 0 ? 1 : 2) : 0;
  ...
  RCLCPP_INFO(
    get_logger(),
    "Reassigned: epoch=%u n=%zu form=%d leader=%u total_cost=%.2fm",
    current_epoch_, n, static_cast<int>(current_formation_),
    leader_it->first, last_total_cost_);
  return result;
}
...
SlotMsg FormationNode::buildAssignmentMsgLocked() const
{
  ...
  // consumers don't have to redo the transform.
  auto leader_it = robot_poses_.find(leader_robot_id_);
  if (leader_it == robot_poses_.end() && !robot_poses_.empty()) {
    leader_it = robot_poses_.begin();
  }
  const PoseXY leader = (leader_it != robot_poses_.end()) ?
  ...
  for (const auto & slot : current_assignment_) {
    ...
    m.slot_type.push_back(slot.slot_type);
  }
  return m;
}
...
std::vector<TargetMsg> FormationNode::buildFollowerTargetsLocked()
{
  ...
  const auto & leader_snap = leader_it->second;
  const PoseXY leader = combat_active_ ? last_anchor_ : leader_snap.pose;
  const Velocity2D leader_vel = combat_active_
    ? Velocity2D{}
    : leader_snap.vel_est.latest().value_or(Velocity2D{});
...
StatusMsg FormationNode::buildStatusMsgLocked() const
{
  ...
  // KPP-1 alignment metrics — use proper frame transform.
