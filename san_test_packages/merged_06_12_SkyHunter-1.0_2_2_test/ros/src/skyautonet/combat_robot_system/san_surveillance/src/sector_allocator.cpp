...
std::vector<SectorAssignment> allocateSectors(const AllocatorInput & in)
{
  ...
  if (!in.threat_bearings_deg.empty() && !out.empty()) {
    const auto & bearings = in.threat_bearings_deg;
    const size_t M = bearings.size();
    const size_t K = out.size();
    const size_t cap = (K + M - 1) / M;
    std::vector<size_t> load(M, 0);
    
    struct Cand { size_t i; size_t j; float d; };
    std::vector<Cand> cands;
    cands.reserve(K * M);
    for (size_t i = 0; i < out.size(); ++i) {
      for (size_t j = 0; j < M; ++j) {
        cands.push_back({i, j, angularDifference(out[i].centreDeg(), bearings[j])});
      } 
    } 
    std::sort(
    ...
    std::vector<int> assign(out.size(), -1);
    for (const auto & c : cands) {
      if (assign[c.i] != -1 || load[c.j] >= cap) {continue;}
    ...
    for (size_t i = 0; i < out.size(); ++i) {
      const float bearing = bearings[assign[i]];
      out[i].sector_start_deg = normalizeAngle(bearing - THREAT_SECTOR_HALF_DEG);
      out[i].sector_end_deg   = normalizeAngle(bearing + THREAT_SECTOR_HALF_DEG);
      out[i].priority  = PRI_THREAT_FOCUS;
      out[i].mode_hint = MODE_TRACK;
    } 
  } 
