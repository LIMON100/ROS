...
{
namespace { constexpr double kDeg2Rad = M_PI / 180.0; }
...
{ 
  ...
  {
    uint8_t mode; float pan, tilt, sweep_range; bool have;
    {
      std::lock_guard<std::mutex> g(cmd_mu_);
      have = have_cmd_;
      mode = cmd_mode_; pan = cmd_pan_deg_; tilt = cmd_tilt_deg_;
    ...
    std_msgs::msg::Float64 p; p.data = state_.pan_deg  * kDeg2Rad;
    std_msgs::msg::Float64 t; t.data = state_.tilt_deg * kDeg2Rad;
    pan_pub_->publish(p);
    tilt_pub_->publish(t);
  }
