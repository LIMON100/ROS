...
private:
  ...
  void onTargetVote(const std_msgs::msg::UInt32::SharedPtr msg);
  ...
  rclcpp::Subscription<std_msgs::msg::UInt32>::SharedPtr vote_sub_;
  ...
  rclcpp::Publisher<combat_robot_msgs::msg::FireSolution>::SharedPtr fire_pub_;
  rclcpp::TimerBase::SharedPtr realloc_timer_;
  rclcpp::Subscription<std_msgs::msg::UInt32MultiArray>::SharedPtr engaged_sub_;
  
  std::set<uint32_t> engaged_ids_;
  double last_engaged_sec_ = 0.0;
  // Params
  ...
  double fire_range_m_ = 10.0;
  double lead_time_s_ = 0.5;
  float  fire_prev_bearing_ = 0.0f;
  uint64_t fire_prev_ts_ = 0;
  float  fire_rate_dps_ = 0.0f;
  float  fire_lead_bearing_ = 0.0f;
  float  fire_range_val_ = 0.0f;
  float  fire_elev_val_ = 0.0f;
  bool   fire_sol_valid_ = false;
  std::map<uint32_t, uint64_t> votes_;
  int    min_votes_ = 1;
  double vote_window_s_ = 1.5;
