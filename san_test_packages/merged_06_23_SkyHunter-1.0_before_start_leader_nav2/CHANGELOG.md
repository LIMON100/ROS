...
- **feat(convoy): 돌발상황 대응 — DCN-2026-029 P4 (nav2 wiring 실증)**. 돌발 장애물 →
  로컬 회피 + nav2 recovery + 참조 재획득. raw `FollowPath`(controller)는 recovery 미호출
  → P4 는 추종자를 **`NavigateThroughPoses`(bt_navigator) 모드**로 돌려 기본 BT 의 recovery
  (clear costmap→backup→spin) + 재획득을 얻는다.
  - `convoy_nav2_follower` `mode` param: `follow_path`(P3) | `navigate_poses`(P4) —
    via-pose 통과 + goal ABORTED/CANCELED 시 `prev_head` 리셋 → 강제 재전송(참조 재획득).
  - `convoy_avoidance.downsample_path`(순수, +pytest 4건) — 조밀 breadcrumb 를 호장 spacing
    으로 솎아 NavigateThroughPoses 경유점화(전역 플래너 과부하 방지).
  - `convoy_demo.launch.py ugv_lidar:=true`(+`ugv_lidar_mode`) — UGV lidar 활성 +
    `/robot_<n>/scan/points` 브리지 → 추종자 로컬 costmap 입력(돌발 인지). 기본 none(RTF 보존).
  - `convoy_nav2.launch.py follower_mode:=navigate_poses` 로 P4 모드 전환.
  - **검증(Jazzy):** 추종자 `mode=navigate_poses` → nav2 ACTIVE → 합성 ref_path(13점) →
    `downsample_path` 4 via-pose → **bt_navigator accepted** `Begin navigating ... through
    4 poses to (3.00, 0.00)`. 총 standalone pytest 33건. 실 회피/recovery 는 gz 필요.

- **feat(convoy): nav2 FollowPath 추종자 골격 — DCN-2026-029 P3 (sim 검증 보류)**.
  리더 참조경로(`/convoy/ref_path/r{id}`)를 nav2 `FollowPath` 로 추종(설계 §4 Option A:
  Go2=global planner, nav2=추종+로컬 회피).
  - `san_operator_tools/convoy_nav2_follower` — `/convoy/ref_path/r{id}` 구독 → 네임스페이스
    `follow_path`(controller_server) 액션. 경로 frame 을 `map` 으로 re-stamp(sim 지상진실
    map≡전역 odom). `convoy_avoidance.should_resend_path`(순수, +pytest 3건)로 head 이동
    ≥ tol 시만 재전송 → controller preempt 억제.
  - `convoy_nav2.launch.py follow_ref_path:=true` 로 로봇별 추종자 기동. convoy_ugv(직접
    cmd_vel)와 `/robot_<id>/cmd_vel` 경합 → 택일(convoy_ugv 는 pure-pursuit fallback 보존).
  - **검증 보류(Linux/CI):** `follow_path` goal accepted·참조 추종(대형 ~3 m)은 Jazzy/Harmonic
    실증 필요(DRAFT PR). ruff + ruff-format clean, py_compile·standalone pytest(29건) 통과.

- **feat(convoy): per-UGV nav2 bringup — DCN-2026-029 P2 (Jazzy bringup 실증)**.
  convoy 추종자(UGV)를 nav2 로 구동하기 위한 bringup. `swarm_nav.launch.py` 패턴 재사용:
  - `san_nav2/config/convoy_nav2_params.yaml` — san_nav2 단일소스 변형(RPP `FollowPath`
    O-1 계승, SmacPlanner2D·rolling costmap 계승), **AMCL 제거**(sim 지상진실), 속도
    0.6 m/s·UGV 풋프린트, odom/scan 상대토픽.
  - `san_operator_tools/convoy_odom_tf` — 지상진실 `/robot_<id>/odom` → tf
    `<ns>/odom`→`<ns>/base_footprint` 중계(convoy tf-free → nav2 tf 트리, **DCN O-2** sim 해소).
  - `san_sim_gazebo/launch/convoy_nav2.launch.py` — convoy_demo 위 **오버레이**(로봇 spawn
    안 함). 로봇별 static `map`→`<ns>/odom` + `convoy_odom_tf` + nav2 스택(controller/planner/
    behavior/bt/smoother/waypoint + lifecycle), `RewrittenYaml(root_key=ns)`, staggered.
  - **검증(Ubuntu 24.04 / ROS 2 Jazzy, convoy_ws):** colcon build 클린 + 노드 import +
    per-UGV nav2 bringup → 전 플러그인 로드, 합성 odom 으로 tf 완성 시 **lifecycle "Managed
    nodes are active"**(전 스택 ACTIVE). **Jazzy 포팅 버그 수정**: 계승 플러그인 lookup 명
    슬래시(`nav2_smac_planner/SmacPlanner2D`·`nav2_behaviors/Spin`·`nav2_bt_navigator/...`)
    → Jazzy 미등록 FATAL → 전부 `::` 형식으로 수정(DCN-2026-027 §4-2 Nav2 churn 실체화;
    base `nav2_params.yaml` 도 동일 잠재버그). 실 goal 항법/대형 추종은 gz 물리 필요(실 RK3588/Linux).

- **feat(convoy): 추종자 nav2 참조경로 발행 — DCN-2026-029 P1**. 리더(`convoy_coordinator`)가
  자기 breadcrumb 궤적(이미 장애물을 회피한 안전 차선)을 누적해, 각 추종자의 슬롯
  (체인상 `SLOT[n]*slot_gap` 호장)부터 head 까지를 `nav_msgs/Path` 로 `/convoy/ref_path/r{n}`
  에 **@5 Hz** 발행. 단일종대라 차선은 공통, 슬롯만 다름(중첩 차선). 순수 기하
  `convoy_avoidance.ref_path_from_trail`/`slot_index`(ROS 무관, 호장 기준)로 분리 +
  standalone 단위테스트 7건. **P1 범위 = 리더측 발행만**(추종자 무변경) — Path 내용·주기
  검증용. `/convoy/target/r{n}` 은 fallback 으로 유지. CONVOY_NAV2_INTEGRATION_DESIGN.md
  를 정식 DCN-2026-029 로 승격.
...
- **fix(san_nav2): nav2 플러그인 lookup 명 슬래시→`::` (ROS 2 Jazzy)**. `config/nav2_params.yaml`
  · `config/nav2_follower_params.yaml` 의 planner(`nav2_smac_planner/SmacPlanner2D`·
  `nav2_navfn_planner/NavfnPlanner`)·behaviors(`nav2_behaviors/Spin` 외 3)·bt_navigator
  (`nav2_bt_navigator/Navigate*Navigator`) lookup 명이 슬래시 형식이라 Jazzy pluginlib
  미등록 → 해당 서버 **FATAL**(planner/behavior/bt 미기동). 전부 `::` 형식으로 수정.
  DCN-2026-027 §4-2 가 예고한 Nav2 API churn 의 실체. **검증(Jazzy):** `ros2 launch
  san_nav2 nav2.launch.py` → controller(RPP)/planner(SmacPlanner2D)/behaviors/smoother
  전 플러그인 정상 로드, FATAL 0. (DCN-2026-029 P2 convoy 검증 중 발견; convoy_nav2_params.yaml
  은 PR #283 에서 선수정.)
