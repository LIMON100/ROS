...
def _spawn_nodes(world, lidar_mode="none"):
    # UGV 4대 + 장애물 실린더를 gz world 에 spawn 하는 ros_gz_sim create Node 들.
    # lidar_mode: 기본 none(RTF 확보). P4 로컬 costmap 검증 시 "low" 등으로 UGV lidar 활성.
    ...
    for n in CHAIN:
        ...
        urdf = xacro.process(
            xacro_file,
            mappings={"robot_ns": ns, "robot_name": ns, "lidar_mode": lidar_mode},
        )
...
def _convoy_nodes(gap, ugv_vmax, leader_vmax, leader_wmax, lidar_only, ugv_lidar=False, nav2_robots=frozenset()):
    bridge_args = []
    for n in CHAIN:
        bridge_args += [
        ...
        # P4: UGV lidar 활성 시 gz gpu_lidar pointcloud 를 ROS 로 브리지 → 각 UGV nav2
        # 로컬 costmap(/robot_<n>/scan/points) 입력. (gz→ROS 방향 '[')
        if ugv_lidar:
            bridge_args.append(
                f"/robot_{n}/scan/points"
...
def _bringup(context, *args, **kwargs):
    ...
    ugv_lidar = LaunchConfiguration("ugv_lidar").perform(context).lower() == "true"
    # ugv_lidar=true → UGV lidar 활성(P4 로컬 costmap). lidar_mode 로 해상도/부하 조절.
    lidar_mode = (
        LaunchConfiguration("ugv_lidar_mode").perform(context) if ugv_lidar else "none"
    )
    ...
    nav2_robots = {
        int(x)
        for x in LaunchConfiguration("nav2_robots").perform(context).split(",")
        if x.strip()
    }
    # actions = _spawn_nodes(world, lidar_mode) + _convoy_nodes(
    #     gap, ugv_vmax, leader_vmax, leader_wmax, lidar_only, ugv_lidar
    # )
    actions = _spawn_nodes(world, lidar_mode) + _convoy_nodes(
        gap, ugv_vmax, leader_vmax, leader_wmax, lidar_only, ugv_lidar, nav2_robots
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "ugv_lidar",
                default_value="false",
                description="P4: UGV lidar 활성 + /robot_<n>/scan/points 브리지 "
                "(추종자 nav2 로컬 costmap 입력). 기본 none(5로봇 RTF 확보)",
            ),
            DeclareLaunchArgument(
                "ugv_lidar_mode",
                default_value="low",
                description="ugv_lidar=true 시 lidar_mode (full|half|low) — low 권장(부하↓)",
            ),
            DeclareLaunchArgument(
                "sim_timeout",
                default_value="120.0",
            ...
            DeclareLaunchArgument(
                "nav2_robots",
                default_value="",
                description="comma list of UGV ids that run their OWN nav2 "
                "(via convoy_nav2.launch.py) — their breadcrumb convoy_ugv is NOT "
                "started, so nav2 owns /robot_<n>/cmd_vel. '' = all breadcrumb.",
            ),
            gz_resource,
            sitl,
            viz,
