...
std::optional<double> toDouble(const std::string & s)
{
  ...
  // Reject NaN / +Inf / -Inf — strtod parses "nan"/"inf" silently.
  // A glitching receiver that emits these would otherwise propagate
  // through parseDmToDeg into lat/lon and poison downstream
  // localization / SLAM.
  ...
  const std::string & hemi)
{
  ...
  // Reject obvious garbage in the integer degrees (e.g. "3599" from
  // a glitching receiver). Lat fits in 90; lon fits in 180. Pad the
  // bound slightly for "359°59.9'" edge but reject "999".
  if (*deg < 0 || *deg > 180) {return std::nullopt;}
  ...
  // Final bounds — even with valid-looking inputs (e.g. lat=89,
  // min=59.999) we want to refuse anything outside the canonical
  // WGS84 ranges. Caller distinguishes lat/lon by hemisphere, but
  // either way |result| ≤ 180 is sufficient because lat-shaped
  // hemispheres (N/S) cap at 90 via the deg check (deg ≤ 90 for
  // N/S in practice). For lon (E/W) we accept up to ±180.
  if (!std::isfinite(result) || result < -180.0 || result > 180.0) {
    return std::nullopt;
  }
  return result;
}
...
std::optional<GgaResult> parseGga(const std::string & sentence)
{
  ...
  // Lat/Lon are required. Enforce hemisphere-specific bounds —
  // parseDmToDeg already rejects NaN/Inf/out-of-range; here we
  // narrow lat to ±90 (lon ±180 is already the parser's bound).
  ...
  if (*lon < -180.0 || *lon > 180.0) {return std::nullopt;}
  r.latitude_deg = *lat;
  r.longitude_deg = *lon;
