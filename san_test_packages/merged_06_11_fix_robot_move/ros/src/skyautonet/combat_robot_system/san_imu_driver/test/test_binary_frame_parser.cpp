...
{
  uint16_t s = 0; for (auto b : v) {
    s = static_cast<uint16_t>(s + b);
  }
  return s;
}
...
// ─── B11: Zero-length payload — minimum valid frame ────────────────────
//
// {sync, len=0, csum_of(len)} = 3 bytes. The parser must accept it
// (some IMU protocols use heartbeat frames with empty payload).
// XOR over csum_region = {0x00} → 0x00.
TEST(BinaryFrameParser, B11_ZeroLengthPayload) {
  BinaryFrameConfig cfg;
  cfg.sync_byte_0 = 0xFA;
  cfg.checksum_kind = ChecksumKind::XorByte;
  cfg.max_payload = 64;

  std::vector<uint8_t> frame = {0xFA, 0x00, 0x00};   // sync, len=0, csum=0
  size_t consumed = 0;
  auto r = parseBinaryFrame(frame, cfg, &consumed);
  ASSERT_TRUE(r.has_value()) << "zero-length payload must be accepted";
  EXPECT_EQ(consumed, 3u);
  EXPECT_TRUE(r->payload.empty());
}

// ─── B12: Partial frame at buffer end (after a valid earlier frame) ───
//
// Two-frame stream where the second frame is mid-truncated. The parser
// is called repeatedly by the driver; this test asserts that one call
// consumes EXACTLY the first complete frame and the truncated tail is
// left in the buffer for the next read. Without this property the
// driver would silently drop the partial frame.
TEST(BinaryFrameParser, B12_PartialFrameAtBufferEndPreservesTail) {
  BinaryFrameConfig cfg;
  cfg.sync_byte_0 = 0xFA;
  cfg.checksum_kind = ChecksumKind::XorByte;
  cfg.max_payload = 64;

  // Frame 1: complete {0xFA, 0x01, 0x55, csum=(0x01^0x55)=0x54}
  // Frame 2: partial  {0xFA, 0x02, 0x01}  — missing 1 byte + csum
  std::vector<uint8_t> buf = {0xFA, 0x01, 0x55, 0x54,
    0xFA, 0x02, 0x01};

  size_t consumed = 0;
  auto r1 = parseBinaryFrame(buf, cfg, &consumed);
  ASSERT_TRUE(r1.has_value());
  EXPECT_EQ(consumed, 4u) << "first frame must be exactly 4 bytes";
  EXPECT_EQ(r1->payload, (std::vector<uint8_t>{0x55}));

  // Simulate driver behavior: erase consumed bytes, retry on the tail.
  buf.erase(buf.begin(), buf.begin() + consumed);
  EXPECT_EQ(buf.size(), 3u);
  size_t consumed_tail = 99;
  auto r2 = parseBinaryFrame(buf, cfg, &consumed_tail);
  EXPECT_FALSE(r2.has_value());
  EXPECT_EQ(consumed_tail, 0u)
    << "truncated tail must NOT be consumed — driver should wait "
    "for more bytes, not drop the partial frame";
}
