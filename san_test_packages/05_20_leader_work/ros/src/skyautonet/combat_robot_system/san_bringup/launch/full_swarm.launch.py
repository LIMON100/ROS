...
def launch_setup(context, *args, **kwargs):
    ...
    pkg_tin3_gz_simulation = get_package_share_directory('san_gazebo')
    pkg_tin3_navigation = get_package_share_directory('san_nav2')
    pkg_skyhunter_comm = get_package_share_directory('san_comm')
    ...
    nodes_to_start.append(IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('san_localization'), 'launch', 'localization.launch.py')
        ),
        launch_arguments={
            'namespace': '',
            'use_sim_time': use_sim_time,
            'use_amcl': 'false'  # GPS mode for outdoor
        }.items()
    ))

    # Leader Node (Produces waypoints for R1)
    nodes_to_start.append(Node(
        package='san_control', executable='leader_node',
        output='screen', parameters=[{
            'use_sim_time': True,
            'initial_formation': int(formation_type)
        }],
        # remappings=[('odom', '/odom_filtered'), ('leader_state', '/leader_state')]
        remappings=[('odom', '/odometry/filtered'), ('leader_state', '/leader_state')]
    ))

    nodes_to_start.append(Node(
        package='human_detector', executable='yolo_detector_node',
    ...
    nodes_to_start.append(Node(
        package='san_control', executable='leadership_manager',
    ...
    for i in range(2, num_robots + 1):
        ...
        off_dist = -4.0 * row  
        drive_off_back = -4.0 * row  
        drive_off_side = 1.0 * side
        ...
        follower_action = TimerAction(
            ...
            actions=[
                ...
                #  Dual-EKF Localization for Follower
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(get_package_share_directory('san_localization'), 'launch', 'localization.launch.py')
                    ),
