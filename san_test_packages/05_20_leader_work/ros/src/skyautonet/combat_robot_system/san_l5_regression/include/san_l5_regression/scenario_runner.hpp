...
private:
    ...
    static bool isLeaderPromotedBy(const LeaderMsg& m, uint32_t robot_id,
                                    uint8_t priority);
    static bool isLeaderPromotedAnyPriority(const LeaderMsg& m,
                                              uint8_t priority);
    static bool isHubPromotedWithFullTakeover(const HubMsg& m,
                                                uint32_t robot_id);
    static bool isLimpModeAlert(const std_msgs::msg::String& m,
