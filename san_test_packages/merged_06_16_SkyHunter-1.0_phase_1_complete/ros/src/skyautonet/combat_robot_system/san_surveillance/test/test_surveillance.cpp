...
TEST(SectorAllocator, A10_ThreatFocusUnionCoverageAtLeast80Percent) {
  ...
  in.mode = SurveillanceMode::Recon;
  in.threat_bearing_deg = -90.0f;   // left flank
  auto out = allocateSectors(in);
...
// ─── DCN-2026-026 C-1 — multi-threat focus ──────────────────────────

TEST(SectorAllocator, A11_TwoThreatsCapThreeTrackersCoverage80) {
  AllocatorInput in;
  in.robots = {{1, RobotRole::Leader, true}, {2, RobotRole::Hub, true}};
  for (uint32_t i = 3; i <= 8; ++i) {
    in.robots.push_back({i, RobotRole::Follower, true});
  ...
  in.mode = SurveillanceMode::Recon;
  in.threat_bearings_deg = {-90.0f, 120.0f};   // two distinct clusters
  auto out = allocateSectors(in);

  // Total trackers capped at 3 (THREAT_FOLLOWER_COUNT), split ≤ 2 per
  // cluster; Leader(1)/Hub(2) must never be re-pointed.
  int n_left = 0, n_right = 0;
  for (const auto & s : out) {
    if (s.priority != 1) {continue;}            // PRIORITY_THREAT_FOCUS
    EXPECT_EQ(s.mode_hint, 1u);                 // MODE_TRACK
    EXPECT_NE(s.robot_id, 1u);
    EXPECT_NE(s.robot_id, 2u);
    const float centre = s.centreDeg();
    if (angularDifference(centre, -90.0f) < 1.0f) {
      ++n_left;
    } else if (angularDifference(centre, 120.0f) < 1.0f) {
      ++n_right;
    } else {
      ADD_FAILURE() << "focus sector at unexpected centre " << centre;
    }
  }
  EXPECT_EQ(n_left + n_right, 3) << "3-follower budget (2 + 1 split)";
  EXPECT_GE(n_left, 1);
  EXPECT_LE(n_left, 2);
  EXPECT_GE(n_right, 1);
  EXPECT_LE(n_right, 2);

  // Survivors redistribute the front band — union coverage holds.
  const float frac = unionCoverageFraction(out);
  EXPECT_GE(frac, 0.80f)
    << "two-threat union coverage = " << (frac * 100.0f)
    << "% — must stay ≥ 80% per SDD §8.6.1 / DCN-2026-026 C-1.";
}

TEST(SectorAllocator, A12_ThreatBearingClustering) {
  // Within 15° → one cluster at the circular mean.
  {
    auto c = clusterThreatBearings({-90.0f, -80.0f});
    ASSERT_EQ(c.size(), 1u);
    EXPECT_NEAR(c[0], -85.0f, 1e-3);
  }
  // Beyond 15° → two clusters, input order preserved.
  {
    auto c = clusterThreatBearings({10.0f, 40.0f});
    ASSERT_EQ(c.size(), 2u);
    EXPECT_NEAR(c[0], 10.0f, 1e-3);
    EXPECT_NEAR(c[1], 40.0f, 1e-3);
  }
  // Cluster cap: a third distinct bearing is dropped.
  {
    auto c = clusterThreatBearings({0.0f, 60.0f, 120.0f});
    EXPECT_EQ(c.size(), 2u);
  }
  // Merge across the ±180° wrap.
  {
    auto c = clusterThreatBearings({175.0f, -175.0f});
    ASSERT_EQ(c.size(), 1u);
    EXPECT_NEAR(angularDifference(c[0], 180.0f), 0.0f, 1e-3);
  }
  // Legacy single-threat path: scalar input folds into the list and
  // keeps the A7/A10 behavior (verified there) — here just the merge.
  {
    AllocatorInput in;
    in.robots = {{1, RobotRole::Leader, true}};
    for (uint32_t i = 3; i <= 8; ++i) {
      in.robots.push_back({i, RobotRole::Follower, true});
    }
    in.threat_bearing_deg = -88.0f;             // legacy scalar
    in.threat_bearings_deg = {-92.0f};          // merges with scalar
    auto out = allocateSectors(in);
    int n_focus = 0;
    for (const auto & s : out) {
      if (s.priority == 1) {
        ++n_focus;
        EXPECT_NEAR(angularDifference(s.centreDeg(), -90.0f), 0.0f, 1e-3);
      }
    }
    EXPECT_EQ(n_focus, 3) << "merged single cluster keeps legacy 3";
  }
}


// ─── DCN-2026-026 C-3 — k-of-n 교전 합의 집계 (V1..V3) ─────────────

TEST(VoteTally, V1_DistinctRobotsKofN) {
  VoteTally t(1500, 15.0f);
  // Two distinct robots confirm the same cluster → k=2.
  t.record(3, 100, -90.0f, 1000);
  t.record(4, 205, -85.0f, 1200);            // different track, same cluster
  EXPECT_EQ(t.countFor(-88.0f, 1500), 2);
  // The same robot voting twice (two tracks) still counts ONCE.
  t.record(3, 101, -92.0f, 1300);
  EXPECT_EQ(t.countFor(-88.0f, 1500), 2);
}

TEST(VoteTally, V2_WindowExpiryAndBinding) {
  VoteTally t(1500, 15.0f);
  t.record(3, 100, -90.0f, 1000);
  t.record(4, 200, -90.0f, 1000);
  EXPECT_EQ(t.countFor(-90.0f, 2400), 2);    // within 1.5 s
  EXPECT_EQ(t.countFor(-90.0f, 2600), 0);    // expired
  // Bearing binding: a vote 40° away belongs to ANOTHER cluster.
  t.record(5, 300, -50.0f, 3000);
  EXPECT_EQ(t.countFor(-90.0f, 3000), 0);
  EXPECT_EQ(t.countFor(-50.0f, 3000), 1);
}

TEST(VoteTally, V3_LatestVoteWinsAndPrunes) {
  VoteTally t(1500, 15.0f);
  t.record(3, 100, -90.0f, 1000);
  t.record(3, 100, -60.0f, 2000);            // same robot+track moves
  EXPECT_EQ(t.countFor(-90.0f, 2100), 0);    // old bearing replaced
  EXPECT_EQ(t.countFor(-60.0f, 2100), 1);
  EXPECT_EQ(t.sizeForTest(), 1u);            // no unbounded growth
}
...
TEST(PatchFrame, PS5_WorldSectorTransformsToCorrectHeadingPerRobot) {
  ...
  in.threat_bearing_deg = 120.0f;        // force focus
  auto sectors = allocateSectors(in);
