  ...
  float step_resolution_m)
{
  ...
  const size_t n_steps = std::max<size_t>(
    1,
  ...
  for (size_t i = 0; i <= n_steps; ++i) {
    ...
    // PATCH 2026-05-13 (C6): apply effective_cost CONSISTENTLY — the
    // v1.5.0 code used effective_cost for max_cost but raw `c` for
    // obstacle/inflated detection, so COST_UNKNOWN tripped the lethal
    // branch (255 >= 254). That made every OOB sample look like a
    // lethal obstacle, contradicting the comment above and the
    // E5_RespectsMapBounds expectation.
    const uint8_t effective_cost = (c == COST_UNKNOWN) ?
      inflated_threshold : c;
    if (effective_cost > r.max_cost) {r.max_cost = effective_cost;}
    if (effective_cost >= lethal_threshold && !first_lethal_seen) {
      r.obstacle_detected = true;
      r.obstacle_distance_m = step_len * static_cast<float>(i);
      first_lethal_seen = true;
    }
    if (effective_cost >= inflated_threshold &&
      effective_cost < lethal_threshold)
    {
