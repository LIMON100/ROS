...
class Coordinator(Node):
    ...
    def pub_ref_paths(self):
        # P1: per-robot reference path @5 Hz for the follower's nav2 FollowPath.
        # Single-column → every UGV's reference lane = the leader planned path
        # (same centerline, 'odom' frame). Gap/slot is handled follower-side in P3.
        path = Path()
        path.header.frame_id = "odom"
        path.header.stamp = self.get_clock().now().to_msg()
        for x, y in self.wps:
