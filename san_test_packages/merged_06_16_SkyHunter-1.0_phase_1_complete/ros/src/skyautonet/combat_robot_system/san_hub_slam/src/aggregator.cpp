...
MultirobotAggregator::MultirobotAggregator(
...
void MultirobotAggregator::setGeometry(
  int width, int height,
  float resolution_m)
{
  width_ = std::max(0, width);
...
bool MultirobotAggregator::applyDelta(
...
bool MultirobotAggregator::applyDeltaRaw(
  const std::string & robot_id,
...
// [SLAM-1] World-frame-aware delta projection. See header for contract.
bool MultirobotAggregator::applyDeltaAt(
  const std::string & robot_id,
  const std::vector<uint8_t> & png_bytes,
  ...
  float delta_resolution_m)
{
  ...
  int dw = 0;
  int dh = 0;
  if (!decodePng(png_bytes, grid, dw, dh)) {return false;}
  return applyDeltaAtDecoded(
    robot_id, grid, dw, dh,
    origin_x, origin_y, origin_theta, delta_resolution_m);
}

bool MultirobotAggregator::applyDeltaAtDecoded(
  const std::string & robot_id,
  const std::vector<uint8_t> & grid, int grid_w, int grid_h,
  double origin_x, double origin_y, double origin_theta,
  float delta_resolution_m)
{
  if (resolution_m_ <= 0.0f || delta_resolution_m <= 0.0f) {return false;}
  ...
  const int dw = grid_w;
  const int dh = grid_h;
  const uint8_t * d = grid.data();

  contributing_.insert(robot_id);

  // A single robot delta must vote each global cell at most once, even if a
  // finer delta resolution maps several of its cells onto the same global
  // cell (otherwise that robot would out-vote others on shared cells).
  std::vector<char> voted(global_.size(), 0);

  const double ct = std::cos(origin_theta);
  const double st = std::sin(origin_theta);
  const double dres = static_cast<double>(delta_resolution_m);
  const double inv_res = 1.0 / static_cast<double>(resolution_m_);

  for (int cy = 0; cy < dh; ++cy) {
    for (int cx = 0; cx < dw; ++cx) {
      const uint8_t v = d[static_cast<std::size_t>(cy) * dw + cx];
      if (v != GLOBAL_FREE && v != GLOBAL_OCCUPIED) {continue;}
      // Cell (cx,cy) local position (origin = delta cell (0,0)), then
      // rotate by theta and translate by the world origin.
      const double lx = cx * dres;
      const double ly = cy * dres;
      const double wx = origin_x + lx * ct - ly * st;
      const double wy = origin_y + lx * st + ly * ct;
      // Floor (not truncate) so negative world coords map correctly.
      const int gx = static_cast<int>(std::floor((wx - origin_x_) * inv_res));
      const int gy = static_cast<int>(std::floor((wy - origin_y_) * inv_res));
      if (gx < 0 || gx >= width_ || gy < 0 || gy >= height_) {continue;}
      const std::size_t idx =
        static_cast<std::size_t>(gy) * static_cast<std::size_t>(width_) + gx;
      if (voted[idx]) {continue;}
      voted[idx] = 1;
      if (v == GLOBAL_FREE) {
        if (free_votes_[idx] < UINT16_MAX) {free_votes_[idx]++;}
      } else {
        if (occupied_votes_[idx] < UINT16_MAX) {occupied_votes_[idx]++;}
      }
    }
  }
  return true;
...
void MultirobotAggregator::recomputeGlobal()
{
  ...
  for (std::size_t i = 0; i < global_.size(); ++i) {
    ...
    if (f > o) {global_[i] = GLOBAL_FREE;} else if (o > f) {global_[i] = GLOBAL_OCCUPIED;}
...
bool MultirobotAggregator::decodePng(
  const std::vector<uint8_t> & png_bytes,
  std::vector<uint8_t> & out, int & width, int & height)
{
  ...
  out.assign(
    decoded.data,
    decoded.data + static_cast<std::size_t>(width) * height);
  return true;
}
