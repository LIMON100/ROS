...
FailureInjector::buildMessage(uint32_t robot_id, const RobotHealth& h) const {
    ...
    if (robot_id == 1) s.robot_role = 1;
    else if (robot_id == 2) s.robot_role = 2;
    else s.robot_role = 0;
...
std::size_t FailureInjector::publishAll() {
    ...
    for (auto& m : snapshot) status_pub_->publish(m);
...
void FailureInjector::publishLteGrade(uint8_t grade, int16_t rsrp_dbm,
