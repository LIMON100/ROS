...
public:
    MultirobotAggregator(int width = 280, int height = 280,
    ...
    // for that cell.
    bool applyDelta(const std::string& robot_id,
                     const std::vector<uint8_t>& png_bytes);

    // Variant useful for tests when the caller already has the
    // decoded uint8 grid (skips PNG round-trip).
    bool applyDeltaRaw(const std::string& robot_id,
