...
> 본 문서는 **`docs/external/Aban_Android_rosbridge_schema_v2.md`
> (DCN-2026-021, 2026-05-23)** 로 대체되었습니다. 외부 발주처(Aban)는
> **schema v2 를 권원**으로 삼아 구현하십시오. 본 문서는 이력 보존용입니다.
>
> 코드와 불일치하는 항목(아래는 schema v2 / 코드가 정답):
> - **포트**: 본문의 `9090` → 실제는 **`9090 + robot_id`** (9091/9092…,
>   `squadron.launch.xml` `rosbridge_port`).
> - **명령 토픽**: 본문 `/operator/*` → schema v2 는 `/emergency_stop`,
>   `/attack_permission`, `/mc/raw_command`.
> - **§7/§10 "EmergencyStop = HMAC" 는 오류** — `EmergencyStop.msg` 에
>   서명 필드가 없고 검증기도 없음. HMAC + Two-key 는 **발사 인증
>   (`san_fire_authorization`) 에만** 적용된다. EmergencyStop 의
>   `operator_id` 는 자체신고일 뿐 서명되지 않는다.

**문서번호**: SAN-WIFI-OPSPEC-001
**발행일**: 2026-05-14
**Rev**: A (SUPERSEDED by Aban schema v2)
...
| 메시지 서명 | **발사 인증만 HMAC** (`san_fire_authorization`). EmergencyStop 은 서명되지 않음 (operator_id 자체신고) | 모든 명령 HMAC + nonce |
...
| 명령 HMAC | ✓ 발사 인증만 (EmergencyStop 은 비서명) | ✓ `san_fire_authorization` |
