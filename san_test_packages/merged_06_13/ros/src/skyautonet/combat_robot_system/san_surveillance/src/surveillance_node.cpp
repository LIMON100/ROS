...
: rclcpp::Node("surveillance_node", opts)
{
  ...
  engaged_sub_ = create_subscription<std_msgs::msg::UInt32MultiArray>(
    "/swarm/engaged_robots", rclcpp::QoS(10).reliable(),
    [this](std_msgs::msg::UInt32MultiArray::SharedPtr m) {
      engaged_ids_.clear();
      for (auto v : m->data) { engaged_ids_.insert(v); }
      last_engaged_sec_ = now().seconds();
    });
...
void SurveillanceNode::onTargetVote(const combat_robot_msgs::msg::TargetConfirmation::SharedPtr msg)
{ 
  ...
  if (!vote_auth_ ||
    vote_auth_->verify(m, msg->hmac_hex, nowMs(now())) !=
    san_fire_authorization::AuthResult::Granted)
  { 
    return;
  } 
  std::lock_guard<std::recursive_mutex> g(state_mu_);
  votes_[msg->robot_id] = nowMs(now());
} 
...
void SurveillanceNode::publishAssignments(const PublishSnapshot & snap)
{
  for (const auto & a : snap.assignments) {
    ...
    if (a.mode_hint == MODE_HINT_SWEEP || a.mode_hint == MODE_HINT_TRACK) {
      ...
      pt.header.stamp = now();
      pt.header.frame_id = "robot_heading";   // body-mounted gimbal
      pt.sequence = sequence_counter_;
      pt.robot_id = a.robot_id;
      
      if (a.mode_hint == MODE_HINT_TRACK) {
        // Active threat → point at it (convert world sector centre to this
        // robot's heading frame).
        float centre_heading = a.centreDeg();
        if (a.frame == SectorFrame::World) {
          ...
          centre_heading = worldToHeading(centre_heading, robot_yaw);
        } 
        pt.target_pan_deg = centre_heading;
        pt.target_tilt_deg = snap.threat_elevation_deg ?
          *snap.threat_elevation_deg : 0.0f;
      } else {
        // No active threat → forward, held.
        pt.target_pan_deg = 0.0f;
        pt.target_tilt_deg = 0.0f;
      }
      pt.speed_dps = 30.0f;
      pt.mode = PT_MODE_TRACK;     // Track-to-target drives gimbal there + holds
      pt.sweep_range_deg = 0.0f;
      pt.timestamp_ms = snap.now_ms;

      // pantilt_pub_->publish(pt);
      const bool engaged_fresh = (now().seconds() - last_engaged_sec_) < 1.0;
      if (!(engaged_fresh && engaged_ids_.count(a.robot_id))) {
        pantilt_pub_->publish(pt);   // skip → engagement owns this gimbal
      } 
    } 
  }
}
