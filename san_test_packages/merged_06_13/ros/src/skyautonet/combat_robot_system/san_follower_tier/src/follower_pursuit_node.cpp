...
public:
  ...
  : rclcpp::Node("follower_pursuit_node")
  {
    ...
    encircle_radius_ = declare_parameter<double>("encircle_radius_m", 5.0);
    encircle_timeout_s_ = declare_parameter<double>("encircle_timeout_s", 2.0);
    map_frame_ = declare_parameter<std::string>("map_frame", "map");
    {
      std::string ns = get_namespace();          // "/robot_2" → "robot_2/base_footprint"
    ...
    target_sub_ = create_subscription<geometry_msgs::msg::PointStamped>(
      "encircle_target", rclcpp::QoS(10).reliable(),
      [this](geometry_msgs::msg::PointStamped::SharedPtr m) {
        encircle_x_ = m->point.x; encircle_y_ = m->point.y;
        encircle_stamp_ = now(); have_encircle_ = true;
      });
...
private:
  void onTick()
  {
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
      "tick id=%d enc=%d encAge=%.2f haveLdr=%d haveOwn=%d ldrAge=%.2f own=(%.1f,%.1f) ldr=(%.1f,%.1f)",
      robot_id_, (int)have_encircle_,
      have_encircle_ ? (now() - encircle_stamp_).seconds() : -1.0,
      (int)have_leader_, (int)have_own_,
      have_leader_ ? (now() - leader_stamp_).seconds() : -1.0,
      own_x_, own_y_, leader_x_, leader_y_);

    if (have_encircle_ && (now() - encircle_stamp_).seconds() < encircle_timeout_s_) {
      onTickEncircle(); return;
    }
  ...
  void onTickEncircle()
  {
    ...
    double dt = 0.05;
    if (last_tick_.nanoseconds() != 0) {
      const double m = (t_now - last_tick_).seconds();
      if (m > 1e-3 && m < 0.5) {dt = m;}
    } 
    last_tick_ = t_now;
   
    if (!have_leader_) {smoother_.reset(); cmd_pub_->publish(cmd); return;}
    geometry_msgs::msg::TransformStamped tf;
    try {
      tf = tf_buffer_->lookupTransform(map_frame_, base_frame_, tf2::TimePointZero);
    } catch (const std::exception &) {
      smoother_.reset(); cmd_pub_->publish(cmd); return;
    } 
    const double own_x = tf.transform.translation.x;
    const double own_y = tf.transform.translation.y;
    const double own_yaw = yawFromQuat(tf.transform.rotation);
   
    // Ring slot around the target (skyhunter math).
    const double tx = encircle_x_, ty = encircle_y_;

    const double pan_rad = wrapPi(std::atan2(ty - own_y, tx - own_x) - own_yaw);
    combat_robot_msgs::msg::PanTiltCommand pt;
    pt.header.stamp = now();
    pt.robot_id = static_cast<uint32_t>(robot_id_);
    pt.target_pan_deg = static_cast<float>(pan_rad * 180.0 / M_PI);
    pt.target_tilt_deg = 0.0f;
    pt.speed_dps = 120.0f;
    // pt.mode = combat_robot_msgs::msg::PanTiltCommand::MODE_FIXED;
    pt.mode = combat_robot_msgs::msg::PanTiltCommand::MODE_TRACK;
    pantilt_pub_->publish(pt);

    const double base_angle = std::atan2(leader_y_ - ty, leader_x_ - tx);
    double off = 0.0;
    switch (robot_id_) {
      case 2: off = -M_PI / 3.0; break;
      case 3: off =  M_PI / 3.0; break;
      case 4: off = -2.0 * M_PI / 3.0; break;
      case 5: off =  2.0 * M_PI / 3.0; break;
      case 6: off = -M_PI; break; 
      case 7: off =  M_PI; break; 
      default: off = 0.0; break;
    } 
    const double a = wrapPi(base_angle + off);
    const double slot_x = tx + encircle_radius_ * std::cos(a);
    const double slot_y = ty + encircle_radius_ * std::sin(a);
   
    san_follower_tier::SlotGains g{kp_linear_, kp_angular_, max_linear_,
      max_angular_, stop_distance_};
    san_follower_tier::SlotCmd sc = san_follower_tier::computeSlotCmd(
      own_x, own_y, own_yaw, slot_x, slot_y, g);
      
    double linear = sc.linear, angular = sc.angular;
    if (sc.arrived) {
      const double face_err = wrapPi(std::atan2(ty - own_y, tx - own_x) - own_yaw);
      angular = std::clamp(kp_angular_ * face_err, -max_angular_, max_angular_);
      linear = 0.0;
    } 
    const bool obs_fresh = have_obstacle_ && (now() - obstacle_stamp_).seconds() < 0.5;
    if (obs_fresh && nearest_front_ < obstacle_brake_dist_) {
      if (nearest_front_ < obstacle_stop_dist_) {
        linear = 0.0;
        angular = (nearest_left_ < nearest_right_) ? -max_angular_ : max_angular_;
      } else {
        const double s = (nearest_front_ - obstacle_stop_dist_) /
          (obstacle_brake_dist_ - obstacle_stop_dist_);
        linear *= std::clamp(s, 0.0, 1.0); 
      }   
    }   
    const std::pair<double, double> out = smoother_.apply(linear, angular, dt);
    cmd.linear.x = out.first;
    cmd.angular.z = out.second;
    cmd_pub_->publish(cmd);
  }
  ...
  bool slot_follow_{false};
  double slot_timeout_s_{1.0};
  double slot_x_{0.0}, slot_y_{0.0};
  bool have_slot_{false};
  rclcpp::Time slot_stamp_;
  std::string map_frame_{"map"}, base_frame_{"base_footprint"};
  std::shared_ptr<tf2_ros::Buffer> tf_buffer_;
  std::shared_ptr<tf2_ros::TransformListener> tf_listener_;
  rclcpp::Subscription<combat_robot_msgs::msg::FollowerTargetMessage>::SharedPtr slot_sub_;

  double encircle_radius_{5.0};
  double encircle_timeout_s_{2.0};
  double encircle_x_{0.0}, encircle_y_{0.0};
  bool have_encircle_{false};
  rclcpp::Time encircle_stamp_;
