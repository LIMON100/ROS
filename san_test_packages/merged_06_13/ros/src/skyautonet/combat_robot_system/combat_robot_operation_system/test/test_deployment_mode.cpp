...
TEST_F(DeploymentModeTest, AliasesParse) {
  // Aliases must match san_operation_control::fromString so a launch
  // argument accepted by one node is accepted by the other. Regression
  // guard for the 'dev' mismatch that fatally crashed this node while
  // san_operation_control booted fine.
  setToken("test-token-abc123");   // 'dev' resolves to DEVELOPMENT
  for (const auto & alias : {"prod", "lab", "dev"}) {
    const bool is_dev = std::string(alias) == "dev";
    auto node = std::make_shared<CombatRobotOperationSystem>(
      makeOpts(alias, /*auth_required=*/ is_dev));
    EXPECT_NO_THROW(node->initialize()) << "alias=" << alias;
  }
}
