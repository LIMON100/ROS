...
def _robot(launch_dir, ns, robot_id, role, x, y, spacing, formation_mode,
           followers='', yaw=0.0, launch_body='true', cruise_speed='1.5', self_range='1.5', z_min='0.15'):
    return IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'formation_cruise_speed_mps': cruise_speed,
            'formation_lidar_self_range_m': self_range,
            'formation_obstacle_z_min_m': z_min,
        }.items())
...
def launch_setup(context, *args, **kwargs):
    ...
    use_go2 = LaunchConfiguration('use_go2').perform(context).lower() in ('true', '1')
    s1_body_flag = 'false' if use_go2 else 'true'
    s1_speed = '0.5' if use_go2 else '1.5'

    s1_self_range = '4.0'  if use_go2 else '1.5'
    s1_z_min      = '0.50' if use_go2 else '0.15'
    ...
    # path_axis: 미션 주행축. 스폰 슬롯의 '횡방향'이 축에 수직이 되도록 배치.
    #   north(기본, 기존 동작) = +y 주행, 횡 = -x(서쪽=+슬롯)
    #   east (POC 활주로)      = +x 주행, 횡 = +y(북쪽=+슬롯)
    path_axis = LaunchConfiguration('path_axis').perform(context).strip().lower()
    ...
    actions.append(TimerAction(
        ...
        actions=[_robot(launch_dir, 's1', 1, 'leader', lx, ly, spacing, fmode,
                        leader_followers, yaw=lyaw, launch_body=s1_body_flag, cruise_speed=s1_speed, self_range=s1_self_range, z_min=s1_z_min)]))

    # ONLY spawn the Go2 body if the toggle is ON!
    if use_go2:
        actions.append(TimerAction(
            period=6.0,
            # period=(6.0 if num <= 1 else 20.0 + (num - 1) * 8.0),
            actions=[IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(launch_dir, 'go2_leader.launch.py')),
                # Ensure z is 0.375!
                launch_arguments={'x': str(lx), 'y': str(ly), 'z': '0.375', 'yaw': str(lyaw)}.items())]))
    ...
    for k, rid in enumerate(range(2, num + 1)):
        ...
        if random_spawn:
            ...
            if path_axis == 'east':
                x, y, yaw = -5.0, slot * 4.0, 0.0   # behind(-x) + opposite-spread(y)
            else:
                x, y, yaw = -slot * 4.0, -5.0, 0.0  # behind + opposite-spread → drives into slot
        else:
            # slot-aligned. spawn_offslot_m>0 이면 슬롯 뒤로(주행축 반대) 이격해 스폰
            # → static form-up 에서 팔로워가 슬롯까지 이동하는 걸 관찰 가능.
            offs = float(LaunchConfiguration('spawn_offslot_m').perform(context))
            if path_axis == 'east':
                x, y, yaw = -offs, slot * spacing, 0.0   # east path: 횡 = +y
            else:
                x, y, yaw = -slot * spacing, -offs, 0.0  # north path: 횡 = -x
        actions.append(TimerAction(
            period=20.0 + k * 8.0,
            # actions=[_robot(launch_dir, f's{rid}', rid, 'follower', x, y, spacing, fmode,
            #                 yaw=yaw)]))
            actions=[_robot(launch_dir, f's{rid}', rid, 'follower', x, y, spacing, fmode,
                              yaw=yaw, cruise_speed=s1_speed)]))
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument('spawn_offslot_m', default_value='0.0',
                              description='static 팔로워를 슬롯 뒤로 이 거리(m)만큼 이격 스폰(form-up 이동 관찰용)'),
        ...
        DeclareLaunchArgument('path_axis', default_value='north',
                              description='미션 주행축 north|east — 스폰 슬롯 횡방향이 이 축에 수직'),
        DeclareLaunchArgument('use_rviz', default_value='true',
                              description='false: rviz + TF통합 relay 끔 (단일호스트 N대 부하 절감; gz 창으로 관찰)'),
        DeclareLaunchArgument('use_go2', default_value='false', description='Set true to use Go2 Dog as s1 leader'),
