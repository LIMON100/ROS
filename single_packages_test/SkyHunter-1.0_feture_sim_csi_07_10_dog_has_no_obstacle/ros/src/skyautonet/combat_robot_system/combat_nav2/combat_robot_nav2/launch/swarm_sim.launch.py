...
def _robot(launch_dir, ns, robot_id, role, x, y, spacing, formation_mode,
             followers='', yaw=0.0, launch_body='true'):
    return IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(launch_dir, 'robot_bringup_sim.launch.py')),
        launch_arguments={
            'robot_ns': ns, 'robot_id': str(robot_id), 'leader_robot_id': '1',
...
def launch_setup(context, *args, **kwargs):
    ...
    actions.append(TimerAction(
        ...
        actions=[_robot(launch_dir, 's1', 1, 'leader', lx, ly, spacing, fmode,
                        leader_followers, yaw=lyaw, launch_body='false')]))
    # Go2 body for the leader (replaces the UGV body s1 no longer spawns)
    actions.append(TimerAction(
        period=6.0,
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(launch_dir, 'go2_leader.launch.py')),
            launch_arguments={'x': str(lx), 'y': str(ly), 'yaw': str(lyaw)}.items())]))
