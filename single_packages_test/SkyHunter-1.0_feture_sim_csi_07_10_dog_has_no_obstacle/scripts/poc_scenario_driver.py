...
CTRL_QOS = QoSProfile(history=QoSHistoryPolicy.KEEP_LAST, depth=1,
...
# STAGES = [
#     ("LINE",       0,  40.0, 3.0),
#     ("COLUMN",     1, 230.0, 3.0),   # column THROUGH obstacles x=100-218 (swerve handles them)
#     ("ENEMY_STOP", 1, 250.0, 5.0),   # freeze on CLEAR ground past the obstacles
#     ("ASSAULT",    4, 320.0, 3.0),   # arc fan-out on CLEAR ground → no jam
# ]

STAGES = [
    ("LINE",       0, 30.0, 3.0),
    ("COLUMN",     1, 50.0, 3.0),   # column THROUGH obstacles x=100-218 (swerve handles them)
    ("ENEMY_STOP", 1, 60.0, 5.0),   # freeze on CLEAR ground past the obstacles
    ("ASSAULT",    4, 80.0, 3.0),   # arc fan-out on CLEAR ground → no jam
]
...
class StopGoDriver(Node):
    def __init__(self, n):
        ...
        self.ctrl_pubs = [self.create_publisher(SwarmControlCommand, f"/s{i}/swarm/control_command", CTRL_QOS)
                            for i in range(1, n + 1)]
