...
def _lidar_filter_node(ns, use_sim_time, robot_id):
    """swarm_lidar_filter — 팀메이트 라이다 마스킹 + /swarm/robot_world_pos 발행.
    nav2_params 의 costmap/collision_monitor 가 rslidar_points_filtered 를 소비하므로
    단일로봇(마스킹 비활성=passthrough)에서도 항상 기동해야 한다. 또한 form-up 의
    world_pos 공급원이라 군집에선 없으면 slot 재배치가 정지한다(과거 수동기동 의존)."""
    return Node(
        package='combat_robot_nav2', executable='swarm_lidar_filter.py',
        name='swarm_lidar_filter', output='screen',
        parameters=[{'use_sim_time': use_sim_time,
                     'robot_id': robot_id,
                     'map_frame': f'{ns}/map' if ns else 'map',
...
def _legacy_stack(context, use_sim_time, with_gnss):
    ...
    with_can = LaunchConfiguration('with_can_reader').perform(context).lower() == 'true'
    can_reader_node = Node(
        package='combat_robot_nav2', executable='can_reader.py', name='can_reader',
        output='screen', parameters=[{'use_sim_time': use_sim_time}]) if with_can else None

    robot_id = int(LaunchConfiguration('robot_id').perform(context))
    lidar_filter = _lidar_filter_node('', use_sim_time, robot_id)
    ...
    nodes = [rsp_node, localization_launch, lidar_filter,
             map_timer, nav_timer, mission_timer]
    if can_reader_node:
        nodes.insert(2, can_reader_node)
    return nodes
...
def _namespaced_stack(context, ns, use_sim_time, with_gnss):
    ...
    # ★기본 with_can_reader:=false 권장(보드): 인자 없는 can_reader 는 GUI 모드라
    # 헤드리스 보드에서 즉사하고, 실제 CAN 은 board_bringup.sh 의 --control 인스턴스가
    # 담당한다. 이 노드는 X 있는 호스트 단독 테스트용.
    with_can = LaunchConfiguration('with_can_reader').perform(context).lower() == 'true'
    can_reader = Node(
        package='combat_robot_nav2', executable='can_reader.py', name='can_reader',
        output='screen', parameters=[{'use_sim_time': use_sim_time}],
        remappings=[('/odom', f'/{ns}/odom'), ('/cmd_vel', f'/{ns}/cmd_vel')]) \
        if with_can else None

    # --- localization (GNSS) — localization.launch.py 를 인라인 + 프레임/토픽 prefix
    #     robot_id/leader_robot_id 는 gps_to_map 의 datum 공유(리더 방송/팔로워 채택)에 필요.
    robot_id = int(LaunchConfiguration('robot_id').perform(context))
    leader_robot_id = int(LaunchConfiguration('leader_robot_id').perform(context))
    loc_nodes = _localization_nodes(
        ns, ekf_params, use_sim_time, with_gnss, robot_id, leader_robot_id)
    ...
    lidar_filter = _lidar_filter_node(ns, use_sim_time, robot_id)

    group = [PushRosNamespace(ns), rsp] + ([can_reader] if can_reader else []) + \
            loc_nodes + [lidar_filter, map_timer, nav_timer, mission_timer]
    return [GroupAction(group)]


def _localization_nodes(ns, ekf_params, use_sim_time, with_gnss,
                        robot_id=0, leader_robot_id=0):
    ...
    if with_gnss:
        ...
        # navsat_transform_node 는 datum/transform 은 계산하면서도 /odometry/gps
        # 위치를 하드 0 으로 발행하는 버그가 있어(주행해도 map 위치 고정) gps_to_map 으로
        # 대체 — /fix 를 datum 기준 ENU 로 직접 변환해 ekf_map(odom0=/odometry/gps_map)에 공급.
        nodes.append(Node(
            package='combat_robot_nav2', executable='gps_to_map_node',
            name='gps_to_map', output='screen',
            parameters=[{'use_sim_time': use_sim_time,
                         'fix_topic': f'/{ns}/fix',
                         'output_topic': f'/{ns}/odometry/gps_map',
                         'map_frame': f'{ns}/map',
                         'base_frame': f'{ns}/base_footprint',
                         # 편대 datum 공유: 리더가 /swarm/datum 방송, 팔로워 채택.
                         'robot_id': robot_id,
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument('with_can_reader', default_value='true',
                              description='launch 내장 can_reader(무인자=GUI모드) 기동. '
                                          '보드는 board_bringup 의 --control 이 담당하므로 false'),
        # --- 네임스페이스 (다중로봇) ---
