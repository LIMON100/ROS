...
void CombatRobotOperationSystem::Assault_statefunc()
{
    RCLCPP_INFO_THROTTLE(
        ...
        *get_clock(),
        1000,
        "Combat Robot Operation System Assault State (Placeholder)");
    pantiltManualControl();
}
...
void CombatRobotOperationSystem::Tracking_statefunc()
{
    ...
    const bool is_protect_run =
        (run_mode_ == RUN_SURVEILLANCE) || (run_mode_ == RUN_DRONE_SURVEILLANCE);

    if (is_protect_run) {
        // Only engage the class this PROTECT mode targets: persons (0) in
        // PROTECT_GENERAL, drones (1) in PROTECT_DRONE. The queue is already
        // class-filtered at accumulation time; this gates the live-lock adopt
        // paths so a wrong-class lock is never adopted.
        ...
        if (protect_active_track_id_ < 0) {
            if (!protect_target_queue_.empty()) {
                ...
                // adopt it. Skip
                // ids the operator already DENIED this session; ids that
                // have been APPROVED are fine to re-adopt because the
                // approved_set lookup in handleFireControl + the raise
                // check will treat them as already-cleared (auto-fire,
                // no fresh prompt).
    ...
    if (sensor_error_state_ != NONE) {
        ...
        if (last_mission_control_command_time_.seconds() < 0.0) {
            ...
            if (shouldRunDemoForCommand(command_id)) {
                ...
                RCLCPP_INFO(
                    this->get_logger(),
                    "Mission control command received: ASSAULT, transitioning to ASSAULT_STATE");
        ...
        if (run_mode_ == RUN_DEMO) {
            ...
            // Hybrid PROTECT drive phase: once the path follower (or
            // operator via CMD_COMPLETE) marks the mission reached, switch
            // from driving to the scan/engage phase.
            uint8_t mission_st;
            {
                std::lock_guard<std::mutex> lock(mtx_mission_);
                mission_st = mission_execution_state_.status;
            }
            if (mission_st == OperationState::MISSION_REACHED) {
                next_state = (run_mode_ == RUN_SURVEILLANCE)
                    ? SURVEILLANCE_STATE
                    : DRONE_SURVEILLANCE_STATE;
                RCLCPP_INFO(
                    this->get_logger(),
                    "Defensive position reached; transitioning to %s for scan/engage",
                    (run_mode_ == RUN_SURVEILLANCE)
                        ? "SURVEILLANCE_STATE"
                        : "DRONE_SURVEILLANCE_STATE");
            }
        ...
        if (mission_control_command_.command_id == MissionControlCommand::IDLE) {
            ...
            // A "wanted-class" target is one this PROTECT mode engages: person
            // (0) for PROTECT_GENERAL, drone (1) for PROTECT_DRONE. Only a
        ...
        if (mission_control_command_.command_id == MissionControlCommand::IDLE) {
            ...
            // Lock missing — debounce before reverting to SURVEILLANCE.
            ...
            if (lost_for >= tracking_lock_lost_grace_sec_) {
                ...
                // grace. Just clear the active id — the approved_set
                // membership (if any) is retained for the rest of the
                // session, and the denied_set keeps DENY decisions sticky.
                if (protect_active_track_id_ >= 0) {
                    RCLCPP_INFO(
                        this->get_logger(),
