...
constexpr uint8_t FORMATION_ARC          = 4;
...
inline FormationSlot formationSlotFor(uint8_t type, int rank)
{
  ...
  switch (type) {
    ...
    case FORMATION_ARC: {
        // 160° 돌격 원호 — 명시적 슬롯표(diamond 방식, 추종 검증대역 ±2~4 m).
        // 안쪽 2대=가깝고 뒤(-3m), 바깥 2대=넓고 앞(-1m) → 전방으로 열린 곡선(호).
        // ※ 호는 팔로워 4대(5-robot)라야 보인다. 3-robot=점2개=직선.
