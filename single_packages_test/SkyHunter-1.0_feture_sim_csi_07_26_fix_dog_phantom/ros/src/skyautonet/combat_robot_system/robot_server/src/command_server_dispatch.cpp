...
void CommandServerNode::handleStateCommand(const StateCommand& s)
{
  ...
  // 엣지 게이트: 대형/그룹/선택이 실제로 변할 때만 SwarmControlCommand 발행.
  // 태블릿 StateCommand 는 heartbeat 성으로 반복 수신되므로 무게이트 재발행 시
  // formation_type=NONE 스트림이 executor 의 설정 대형을 나란히로 리셋한다(실측).
  // transient_local 라치가 late-join 구독자에게 마지막 명령을 전달하므로
  // 반복 발행은 불필요.
  bool swarm_changed = false;
  {
    std::lock_guard<std::mutex> lock(swarm_cmd_gate_mutex_);
    swarm_changed = !swarm_cmd_published_once_ ||
      swarm.formation_type != last_swarm_cmd_.formation_type ||
      swarm.formation_number != last_swarm_cmd_.formation_number ||
      swarm.grouping_index != last_swarm_cmd_.grouping_index ||
      swarm.selected_robot_count != last_swarm_cmd_.selected_robot_count ||
      swarm.selected_robot_ids != last_swarm_cmd_.selected_robot_ids;
    if (swarm_changed) {
      swarm_cmd_published_once_ = true;
      last_swarm_cmd_ = swarm;
    }
  }
  if (swarm_changed) {
  ...
  if (swarm_changed && m_pub_local_swarm_control_) m_pub_local_swarm_control_->publish(swarm);
}
