...
DetectionProcessor::DetectionProcessor(hailo_param_t hailo_param, std::string label_text_path) {
    m_hailo_engine = std::make_shared<hailo::HailoNmsDetector>(hailo_param.model_path, hailo_param.box_thres);
...
std::vector<hailo::util::object_t> DetectionProcessor::hailo_process(cv::Mat& image) {
    ...
    cv::Mat input_image;
        cv::resize(image, input_image, cv::Size(m_hailo_engine->get_input_size().width, m_hailo_engine->get_input_size().height));
