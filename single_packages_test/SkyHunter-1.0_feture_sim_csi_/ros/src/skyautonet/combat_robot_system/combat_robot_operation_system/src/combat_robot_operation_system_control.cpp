...
void CombatRobotOperationSystem::handleFireControl()
{
    ...
    // PROTECT modes: Tracking_statefunc owns the request-flag lifecycle
    // (raises on new track_id, clears on APPROVE/DENY). The command_server
    // delivers APPROVE/DENY exactly once and then resets to NONE, so we
    // also keep firing while the latched approved_track_id matches the
    // current active one — without touching request_attack_permission_.
    const bool is_protect_run = (run_mode_ == RUN_SURVEILLANCE) ||
