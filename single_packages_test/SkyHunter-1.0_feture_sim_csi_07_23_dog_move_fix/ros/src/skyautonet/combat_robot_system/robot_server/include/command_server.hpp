...
private:
    ...
    // SwarmControlCommand 엣지 게이트: 태블릿 StateCommand 는 상태 heartbeat 처럼
    // 반복 수신되므로, 대형/선택이 실제로 변할 때만 발행한다(transient_local 라치가
    // late-join 을 커버). 무게이트 재발행은 formation_type=NONE 스트림이 executor 의
    // one-shot 대형을 나란히로 리셋하는 실측 버그의 발생원.
    std::mutex swarm_cmd_gate_mutex_;
    bool swarm_cmd_published_once_{false};
    SwarmControlCommand last_swarm_cmd_{};
